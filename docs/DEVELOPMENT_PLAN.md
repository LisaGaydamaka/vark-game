...
The lab uses one real acoustic space, the real player/session path, a cover wall, and **NEAR / HEARD THROUGH COVER / MARGINAL / INAUDIBLE** positions. The speaker repeats one resource-authored proof line every three seconds so the player can move between cases without a separate dialogue UI.

**Done when:** a line cannot appear before its one semantic `gameplay.sound` event crosses the controlled acoustic consequence boundary; during the same active utterance the words continuously fade as current propagated strength falls, disappear at/below hearing threshold, and can reappear if the player returns to audible space before expiry, all without another semantic sound event; representative visual cover does not erase acoustically audible words; expiry clears stale text; the source text/lifetime comes through the tiny line-ID/data Resource rather than guard AI or hardcoded production dialogue; diagnostics distinguish semantic queued/heard counts from live presentation propagation; and no conversation tree, writer tooling, localization framework, voice playback authority, awareness behavior, or Phase 12 dialogue system is pulled forward.

**Automated:** implemented — the Speech suite is wired into `tests/run_all_tests.gd`. It verifies Development Launch exposes Speech Lab, the proof text/lifetime comes from a valid `VarkSpeechLine` Resource, the lab launches through Application → WorldSession → Player → existing acoustic graph, the world-space label renders through real visual cover, and near speech stays hidden until the one semantic sound crosses the stable consequence boundary. A single cover-started utterance is then followed while the player moves through several farther positions: fill and outline alpha must remain equal, every sampled alpha must decrease continuously while queued/heard semantic counts remain unchanged, several closely spaced samples near the hearing threshold must form small monotonic steps toward zero rather than a late jump, the same utterance must hide at the inaudible marker, and it must reappear when the player returns to cover range without another semantic event. A short duplicate line also proves expiry removes the words.

**Manual:** pending corrective recheck — validator: **Windows x64 user/playtester**. Launch **Development Launch → Speech Lab**. Wait for one utterance while standing at **HEARD THROUGH COVER**, then **during that same line** walk slowly toward **MARGINAL** and **INAUDIBLE**. The entire glyph — white fill **and black outline together** — must fade continuously, with the last part easing gradually all the way to nothing rather than leaving a dark outline or making a visible final jump. Do **not** wait for the NPC to speak again. If you turn back and regain audible strength before the same utterance expires, the whole glyph should smoothly reappear. No new line/hearing event should be needed for those presentation changes. Also confirm the cover wall can still hide the NPC visually while the currently audible words remain readable above its world position.
