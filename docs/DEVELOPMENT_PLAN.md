...
- carpet is `quiet` at 0.20, stone is `normal` at the retuned 0.45, tile is `loud` at the retuned 0.90, and crouch/sprint remain multiplicative gait modifiers on top of the selected surface tier. This adds the missing loud tier without changing the accepted carpet/stone baseline.
...
**Done when:** authored surface profiles can select only `quiet`, `normal`, or `loud`; canonical strength is derived from that tier rather than arbitrary per-surface tuning; the real Integrated Slice exposes carpet/stone/tile examples; standing carpet emits quiet `footstep.carpet` at 0.20, stone emits normal `footstep.stone` at 0.45, tile emits loud `footstep.tile` at 0.90, gait modifiers preserve the tier ordering, and the existing guard acoustic listener receives all three through the unchanged gameplay-sound path.
