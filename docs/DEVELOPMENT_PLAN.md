...
**Manual:** pending — validators: **Windows x64 user/playtester** for acoustic intuition and **Windows mapper/user with TrenchBroom 2026.2** for authoring cost. In **Development Launch → Acoustic Lab**, trigger the blue/green/red probes and compare listener labels with the open room, closed/open ordinary door, disconnected room, and around-corner route. The same source should become materially easier to hear through the real door as it opens; the disconnected room must stay silent; the corner route should feel plausible rather than like straight-line radius hearing. Confirm movement/look/interact remain normal. The first mapper pass found a real acceptance failure: acoustic-space properties were editable but the configured volume had no visible borders. After refreshing the Vark TrenchBroom config, retest a disposable `vark_acoustic_space`: its hollow bounds cage must be visible, editing each mapper half-extent must visibly resize the corresponding axis, and save → close/reopen must retain the same volume. Then place/edit a `vark_acoustic_portal`, save/reimport, and confirm the authored topology survives without generated-output hand edits. Exact attenuation/transmission tuning remains provisional until this playtest is accepted.
