...
**Automated:** corrective coverage implemented — the **Phase 3 Integration** suite remains wired into `tests/run_all_tests.gd`. In addition to the original component/nav/door/acoustics/speech/Junk/exposure/objective checks, it now requires exactly one visible rendered `Light3D` in the slice's gameplay `World3D` and requires that node to be the real shadow-casting `VarkGameplayLight`; verifies a standing stone step emits its full `0.52` semantic strength and is heard in the representative cross-room open-door case; fully crouches the real player capsule/head and verifies the same stone surface emits `0.52 × 0.45` and becomes muted in that same acoustic case; then, after the real patrol/door-use proof is complete, pauses only guard locomotion for the deterministic **CROUCH COVER** comparison, places guard/player across that cover, proves standing remains gameplay-lit and visible, fully crouches the same player, proves exposure is still above the visual threshold while the live head target drops by more than 0.40 m, and requires the physical LOS blocker to be exactly `CrouchCover`. The isolated comparison prints its measured exposure/LOS/target geometry if any condition fails.
