...
A development mission-package **Guard/Nav Lab** builds its structural floor/wall/offset doorway from the real FuncGodot `.map` source, then bakes a Godot `NavigationMesh` from the imported static collision geometry using the Godot 4.7 source-geometry → bake API. The authored A/B points lie on opposite sides of a wall while the only opening is deliberately offset, so a valid route must bend through imported architecture rather than moving directly between coordinates. The 1.30 m ordinary visual leaf closes exactly between the authored jambs with no side voids. The shared ordinary-door collision contract remains full-width; only this exact-fit development fixture duplicates that leaf collision and leaves 4 cm of physical sweep clearance at each vertical edge so mapper-fit tolerance does not become a global door change. The guard's authored 2.0 m door-use distance is only a relevance cap for its configured ordinary door, not an automatic OPEN trigger. While APPROACHING, the ordinary door exposes the real horizontal sweep radius of its current physical leaf. The guard combines that with its own navigation/body radius and watches its next intended physics step; only when that next step would enter the resulting safe swing-clearance boundary does it enter WAITING_OPEN, send the existing idempotent `request_open()` intent, and wait for the door's authoritative OPEN state. This keeps the trigger tied to imminent route obstruction while stopping outside the leaf's opening sweep. Temporary physical obstruction may latch an OPEN request, but while the passage is still required the guard retries that OPEN intent; once the blocker leaves and the door opens, patrol resumes. Door traversal is explicit and local to this primitive consumer: **APPROACHING → WAITING_OPEN → CROSSING → CLEAR**. The ordinary door owns the physical passage-occupancy query. While APPROACHING, a non-open door may continue closing in front of the guard without an immediate counter-request; WAITING_OPEN starts only when the next intended step would enter the door's real sweep-plus-body clearance. Once WAITING_OPEN has started, the same idempotent OPEN intent is retried until authoritative OPEN. Once the guard's real body occupies the passage, CROSSING suppresses OPEN counter-requests so a player-commanded close advances until physical contact and remains stably blocked; after the guard exits, CLEAR prevents reopening a door that merely closes behind it. Player `interact()` remains the ordinary toggle. There is no guard-specific door or second navigation truth.
...
**Done when:** one guard is spawned through the Vark TrenchBroom/FuncGodot authoring path; two authored patrol markers drive a deterministic A↔B patrol; its NavigationAgent route is shaped by imported structural geometry rather than straight coordinate interpolation; the same ordinary door is opened only through the existing door-side navigation seam and physical door state; a temporary blocker cannot permanently strand the guard once the opening becomes clear; a player may close a door in front of an approaching guard without an immediate AI reopen, but once the guard's next intended step would enter that door's real sweep-plus-body clearance it stops outside the opening sweep and requests OPEN instead of stranding itself against the leaf; player closing against a guard already in the frame reaches physical contact and remains stably blocked rather than being countermanded by guard door-use logic; the closed 1.30 m leaf fits the authored doorway without left/right voids; a representative map edit/reimport produces a fresh usable navigation mesh and updated patrol target; useful navigation/route diagnostics exist; and no awareness/search/combat/dialogue/final-animation framework is pulled forward.

**Automated:** required — the Navigation suite is wired into `tests/run_all_tests.gd`. It must prove FGD export of the real guard/patrol entities and the 2.0 m authored door relevance cap, exact 1.30 m Guard/Nav Lab visual jamb spacing around the ordinary leaf, fixture-local physical sweep clearance without changing the shared ordinary-door collision leaf, launch through the production mission/session path, nonempty runtime navmesh baking from imported FuncGodot static geometry, a route that detours through the authored offset opening, a guard request/wait triggered at the real leaf-swing-plus-body clearance boundary before the guard enters the opening sweep, a complete A↔B patrol, one logical ordinary-door use, a player-close window in front of an approaching guard with no immediate AI reopen followed by OPEN only when the guard's next step would enter the real swing-clearance boundary, player-commanded closing against a CROSSING guard advancing to contact and latching without an AI OPEN counter-request, real-player obstruction followed by guard OPEN retry/recovery, and a disposable authored patrol-point edit followed by fresh mission rebuild/nav bake/patrol/door traversal. The Application door regression separately proves the shared full-width leaf plus `request_open()` idempotence across blocked opening recovery without toggle reversal or duplicate use sound. Existing authoring/application/props/acoustics/movement suites remain authoritative.

**Manual:** pending corrective recheck — validator: **Windows x64 user/playtester**. The previously handed-off patrol, detour, exact-fit doorway, guard opening, real-player opening obstruction/retry, recovery, and normal movement/look checks were reported working. Remaining recheck: launch **Development Launch → Guard/Nav Lab** and test both sides of the traversal contract. **(1)** Let the guard open the door and enter the doorway, then press ordinary interact to close while the guard is still in the frame: the leaf must close normally until physical contact, remain stably partially closed while obstructed, and show no twitching/reopening/clipping. **(2)** On another pass, start closing the door in front of the guard while it still has clear walking space before the frame: the door must continue closing without an immediate AI reopen. Let the guard keep approaching; shortly before it would enter the leaf's swing/contact zone, it should stop outside the sweep, request/open that same door, then continue through instead of walking into the leaf or standing indefinitely. The guard remains a real physical blocker; no suspicion, pursuit, combat, voiced dialogue, or finished guard presentation is expected.
