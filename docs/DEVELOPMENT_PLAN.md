...
- active search now has a Thief-style execution cadence rather than continuous running: each search leg uses a slow, confidence-sensitive movement speed; arrival resolves a deterministic varied sequence of pause → directed turn → held look → optional additional look(s) → departure pause; the old sinusoidal in-place sweep is removed;
...
- investigation uses a deliberate near-patrol pace, search is slower, and confirmed pursuit is substantially faster. Navigation intent therefore reads physically as caution versus chase rather than using one universal run speed;
