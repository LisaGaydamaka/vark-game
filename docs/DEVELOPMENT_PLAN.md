...
**Done when:** authored surface profiles can select only `quiet`, `normal`, or `loud`; canonical strength is derived from that tier rather than arbitrary per-surface tuning; the real Integrated Slice exposes carpet/stone/tile examples; standing carpet emits quiet `footstep.carpet` at 0.09, stone emits normal `footstep.stone` at 0.45, tile emits loud `footstep.tile` at 0.90, gait modifiers preserve the tier ordering, and the existing guard acoustic listener receives all three through the unchanged gameplay-sound path.
