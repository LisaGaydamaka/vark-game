...
**Current status:** implementation and deterministic regression coverage are checked in on `test`. The first asymmetric vertical-city playtest package is preserved unchanged as **Representative Stealth**. A second, separate **Representative Stealth — Ledge City** package now stress-tests the full accepted traversal envelope instead of only conservative mantle steps: the first required edge is above standing reach, repeated roof transfers rise about 1 m while requiring jump→catch/hang→mantle, dedicated same-height gaps require ledge-to-ledge directional jumps, the key roof reaches roughly 6.4 m, and the shared objective apex reaches roughly 14.2 m. Both packages retain the same representative stealth systems but own distinct mission/save identity. Keep this item `[~]` until post-push CI and focused user/playtester acceptance of the new traversal-stress variant are successful.
