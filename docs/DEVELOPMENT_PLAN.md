...
- `VarkSurfaceProfile` is an authored Resource that owns the stable semantic `surface_id` plus exactly one of three authored loudness tiers: `quiet`, `normal`, or `loud`. Canonical footstep strengths are derived from the tier (`0.09`, `0.45`, `0.90`) rather than freely authored per surface. The current sound kind remains derived as `footstep.<surface_id>`; there is no duplicate authored kind or strength field;
...
- carpet is `quiet` at the near-silent 0.09, stone is `normal` at 0.45, tile is `loud` at 0.90, and crouch/sprint remain multiplicative gait modifiers on top of the selected surface tier. The real slice guard uses a second baseline investigation gate for footsteps: while calm/suspicious, a heard footstep must have at least 0.22 source strength as well as enough propagated strength. This makes stone sneak (0.2025) noticeable-but-suspicious-only from a calm baseline even at point blank, while stone walk/run and tile sneak/walk/run remain investigation-capable. Once independent stronger evidence has already put the guard into investigating/searching, 5.4's heightened-attention interpretation may promote that same locally heard stone-sneak source; the surface did not get louder and acoustic reach did not change. Carpet run peaks at 0.1215, below the slice guard's tuned 0.16 hearing floor before attenuation, so all carpet movement remains almost silent. The 0.16 physical listener floor is deliberately a little less sensitive than the previous 0.14 value; it shortens hearing reach without changing source strengths, acoustic attenuation, or the separate 0.16/0.22 investigation interpretation gates;
...
**Done when:** authored surface profiles can select only `quiet`, `normal`, or `loud`; canonical strength is derived from that tier rather than arbitrary per-surface tuning; the real Integrated Slice exposes carpet/stone/tile examples; carpet sneak/walk/run remain below ordinary hearing, stone sneak from a calm/suspicious baseline can create suspicion but cannot by itself trigger investigation at any distance, stone walk/run can investigate, tile sneak/walk/run can investigate, gait modifiers preserve the tier ordering, and all three route through the unchanged gameplay-sound/acoustic-listener path. Landing after a jump or fall emits exactly one sound at the same surface-relative strength as running. Successful mantle completion emits exactly one stance-scaled ordinary step instead—walking while standing, crouched/sneaking while crouched—and a cancelled mantle that releases to air still produces the later hard landing normally. A guard already in investigation/search may react more strongly to the same heard stone-sneak source through 5.4 state-dependent attention without changing the emitted sound contract.

**Automated:** the authoritative Gameplay Noise suite checks the closed three-tier profile contract, real slice wiring including the tuned 0.16 production guard hearing floor, semantic sound routing, gait scaling, loudness-meter reporting, burst-movement cadence, a controlled normal-airborne → grounded transition, standing/crouched successful mantle handoffs plus cancelled-mantle-to-air fallback, and the hearing-only reaction matrix with awareness vision frozen: carpet remains below hearing for sneak/walk/run; close stone sneak from a calm/suspicious baseline is heard but repeated steps remain suspicious-only below the 0.22 baseline investigation source floor; stone walk/run investigate; a stone landing equals stone-running strength and investigates; tile sneak/walk/run investigate. The Awareness suite separately proves that prior stronger evidence can lower only the interpretation floor while investigating/searching. The Phase 3 Integration suite also requires all three tiered surfaces in the production slice. Post-push CI must remain green.

**Manual:** required — while fully out of sight and beginning from calm, verify that overall hearing reach feels slightly shorter than before, then verify the matrix directly: carpet should be almost silent for sneak, walk, and run; stone sneak may produce a mild suspicion tell when extremely close but must not by itself promote a calm/suspicious guard to investigation; stone walk/run should provoke investigation at useful close/medium ranges; tile should feel dangerous even while sneaking, with walk/run stronger still. Repeat stone sneak very close to a calm/suspicious guard several times to ensure proximity or repeated weak steps do not cross the baseline investigation ceiling. Separately, after a stronger cue has already established investigation/search, a very close stone sneak may now be treated as useful new evidence by the heightened-attention rule. Jump or step/fall from a ledge onto carpet, stone, and tile and verify the landing is as loud/reactive as running on that same surface, without an obvious double footstep on contact. Then mantle onto a surface while standing and while crouched: the completion contact should sound/react like one walking step or crouched/sneaking step respectively, never like a running landing. Deliberately cancel/release a mantle into a fall and verify that the eventual impact is still the ordinary loud landing. Acceptance should focus on NPC behavior, not the debug meter alone.
...
**Manual:** required — user/playtester runs the Integrated Slice and verifies pursuit/search handoff first: while confirmed and visible, move laterally along a wall/ledge, through a doorway, around a corner edge, briefly behind a pillar, across a light/dark boundary, and just outside ordinary view range/FOV. Brief or predictable loss must remain ALERTED and continue toward the last useful reachable approach; it must not instantly become slow search. If the player stays gone, the guard should run to/check that last-known region and only then become SEARCHING. While visibly pursued, throw/make another sound elsewhere and verify it does not pull the guard off the confirmed chase; after genuine visual loss, a strong nearby footstep/landing may update the trail without forcing a distraction stare. Then verify the existing search feel: slow/cautious movement, spaced stops, deliberate pauses/looks, wide persistent coverage, no hidden-player steering, immediate reacquisition/new-evidence response, and eventual recovery. While actively investigating/searching, make a very close stone-sneak or similarly marginal local cue that a calm guard would only question: it should now interrupt the current action, immediately turn/hold on the source, and then recenter investigation without implying knowledge of anything the guard did not really hear/see. Also repeat from an elevated/lowered ledge or climbing position: visible lateral motion should keep alert pursuit with a reachable floor/route approach; calm guards must not gain magical overhead vision and solid floors/walls must still block sight.
