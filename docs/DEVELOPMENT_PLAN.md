...
**Automated:** the accepted 6.5 control-ID/emitter/collision/save behavior is reopened only for fixture self-lighting/material ownership. Exact-head validation must prove the body retains the same authored material identity/albedo/shading across ON/OFF, a private unshadowed fixture-only fill is nonzero while ON and zero while OFF, only the body carries that private render layer, and the normal world/gameplay emitter behavior remains unchanged. The Application suite proved each fixture asset authors the real child `OmniLight3D` emitter inside its bright glass rather than at the mapper origin, owns solid layer-1 collision, and intersects the real player's collision shape at contact distance. Gameplay exposure samples from that same child-emitter position while excluding only the fixture's own solid body from source rays; the existing visibility and integrated-slice suites remained green, including the single rendered shadow-light contract. The run ended with `ALL APPLICATION TESTS PASSED`, `ALL VISIBILITY TESTS PASSED`, and `ALL TEST SUITES PASSED`. The authoring suite still proved mapper-facing `vark_gameplay_light`/`vark_switch` control-ID wiring through real FuncGodot source. The Application suite additionally proved the reusable imported fixture asset keeps its physical body present, shows an authored bright/emissive lit surface with nonzero emitter energy while ON, leaves the model and dark/non-emissive glass visible with zero emitter energy while OFF, and quickload restores that same dark-visible OFF state without hiding the object. Visibility and all other suites stayed green; the run ended with `ALL AUTHORING TESTS PASSED`, `ALL APPLICATION TESTS PASSED`, `ALL VISIBILITY TESTS PASSED`, and `ALL TEST SUITES PASSED`. The authoring suite proved the exported Vark TrenchBroom FGD exposes `vark_gameplay_light` and `vark_switch` linked only by `control_id`, and a real FuncGodot build created two saved gameplay lights plus one switch from mapper source without Godot-side node-path wiring. The Application suite proved one F toggles every light in the group, derived lever presentation follows the group, real gameplay exposure changes, direct extinguish changes the same persistent light truth, quicksave stores only light state, deliberate post-save divergence is possible, and quickload restores lights plus derived switch pose without event replay. The run ended with `ALL AUTHORING TESTS PASSED`, `ALL APPLICATION TESTS PASSED`, and `ALL TEST SUITES PASSED`.

**Manual:** user/playtester. Development Launch → **Switch/Light Lab**. Stand facing the wall switch: it should highlight normally. Press F once; both room emitters must disappear and the light gem/exposure must fall, while each lamp object remains visible with its glass/lit surface visibly dark and the imported lever moves to its off pose. Compare the metal/frame before and after: its base colors/material must not switch with the light state; when ON it should instead be illuminated naturally by the lamp's own fixture-local light and any other scene lights. Press F again to restore both. Walk to the separate right-hand lamp; confirm you cannot walk through its housing, then aim at its fixture and press F to extinguish it directly. It must stop contributing visible/gameplay light through the same state, not leave a visually dark but gameplay-bright mismatch. Movement/look and ordinary interaction must remain normal.
