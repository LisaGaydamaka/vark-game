...
- a Phase 3.5 ordinary-prop implementation with single-slot carried Junk HUD presentation, F throw / R gentle release, real always-upright dynamic rigid-body translation while moving, frozen settled support/stack behavior, hard-edged external-model shading, and semantic capture/reconcile seams; Phase 3.5 remains `[~]` pending the final Windows x64 user acceptance pass and resolution of the post-mantle freeze
...
6. **Post-mantle freeze evidence.** While the user-reported intermittent freeze remains unresolved, the Player carries a focused development trace. It prints a startup heartbeat, arms whenever mantle ownership is observed, and writes `user://post_mantle_freeze_trace.json` after sustained held-WASD movement stalls. The detector deliberately covers four failure classes instead of assuming a clean grounded exit: raw WASD held while the application command goes neutral; mantle ownership remaining active with negligible movement; traversal exiting without support reacquisition; and ordinary post-mantle locomotion being requested but resolved to no movement. The trace records input, traversal/support/step state, velocity channels, displacement, and contact-solver evidence for root-cause diagnosis.
