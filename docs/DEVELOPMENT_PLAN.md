...
**Automated:** pending exact-head post-push validation after the mapper acceptance pass exposed that `vark_prop.prop_id` was authored as a `StringName` default and therefore omitted by FuncGodot's FGD exporter even though synthetic `.map` import still worked. The project-side correction keeps runtime `VarkOrdinaryProp.prop_id` as `StringName` but changes only the FGD resource default to ordinary `String`, which FuncGodot exports as `prop_id(string)`. The Authoring regression now requires that exact exported marker in addition to the existing variant-choice/import/runtime assertions. A full audit of project-owned `authoring/fgd/*.tres` found no other mapper-facing `StringName` defaults.
