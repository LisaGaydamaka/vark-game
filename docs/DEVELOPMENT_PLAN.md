...
A development-only **Speech Lab** reuses the accepted Phase 3.6 acoustic authority instead of inventing subtitle distance/LOS rules. The placeholder NPC speaker owns a tiny `VarkSpeechLine` Resource with stable `line_id`, typed world-space text, semantic `sound_kind`, and gameplay sound strength. Speaking a line first hides any previous text, then queues the existing `gameplay.sound` fact through the authoritative `WorldSession`. The player carries a normal `VarkAcousticListener`; only its existing `gameplay_sound_heard` result may make the words visible.

The words are a billboarded world-space `Label3D` above the speaker with depth testing disabled. That means visual cover does not erase words the player has acoustically heard: the acoustic graph remains the authority for whether speech exists for the player, while presentation is intentionally readable through cover. A simple provisional opacity mapping uses propagated-strength / hearing-threshold ratio so marginal audible speech fades but remains readable. If the acoustic result is inaudible, the line remains hidden even though the semantic speech sound was emitted.
...
**Done when:** a near line appears above the source only after the controlled acoustic consequence pass reports it heard; the same line remains readable through representative visual cover when acoustics still says it is heard; marginal audible speech has lower opacity than near speech; inaudible speech is hidden; the source text comes through the tiny line-ID/data Resource rather than guard AI or hardcoded production dialogue; diagnostics expose queued/heard counts and the last acoustic perception; and no conversation tree, writer tooling, localization framework, voice playback authority, awareness behavior, or Phase 12 dialogue system is pulled forward.

**Automated:** implemented — a new Speech suite is wired into `tests/run_all_tests.gd`. It verifies Development Launch exposes Speech Lab, the proof text comes from a valid `VarkSpeechLine` Resource, the lab launches through Application → WorldSession → Player → existing acoustic graph, the world-space label is configured to render through visual cover, near speech stays hidden before the stable consequence pass and appears after the player listener hears it, a real wall blocks the player-to-speaker vision ray while the acoustically heard line still appears, marginal speech is visibly faded relative to near/cover cases, and an inaudible emission leaves the text hidden.

**Manual:** pending — validator: **Windows x64 user/playtester**. Launch **Development Launch → Speech Lab**. The placeholder speaker repeats one line every three seconds. At **NEAR**, the words should be clear above the speaker. Move to **HEARD THROUGH COVER** so the wall blocks the NPC visually: if the line remains acoustically audible, the words should still appear/read through the wall. At **MARGINAL**, the words should still appear but noticeably fainter. At **INAUDIBLE**, the next utterance should disappear entirely. The important judgment is whether “heard words remain spatially attached to the NPC even through cover; unheard words do not exist on screen” feels natural rather than subtitle-like or clairvoyant.
