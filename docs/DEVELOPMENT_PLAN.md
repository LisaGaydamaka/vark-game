...
**Current status:** implementation and deterministic regression coverage are checked in on `test`. Playtest correction removes newly-added raw-model import-cache dependencies from the window/street-lamp presentations, defers F9/F10 world replacement outside input dispatch, rebuilds the representative map at the same footprint as a dense multi-room/multi-passage compound with a raised west-window climb route, upper catwalk/perches, keyed east room chain and cover alcoves/pillars, and corrects the representative mapper lights so their rendered emitter color is not collapsed to near-black by FuncGodot byte-RGB parsing. Post-push CI is green for this corrected production build; keep this item `[~]` until the revised focused two-route user/playtester acceptance below is successful.
