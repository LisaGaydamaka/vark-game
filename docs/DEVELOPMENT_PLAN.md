...
- `VarkSurfaceProfile` is an authored Resource that owns the stable semantic `surface_id` plus exactly one of three authored loudness tiers: `quiet`, `normal`, or `loud`. Canonical footstep strengths are derived from the tier (`0.09`, `0.21`, `0.90`) rather than freely authored per surface. The current sound kind remains derived as `footstep.<surface_id>`; there is no duplicate authored kind or strength field;
...
- carpet is `quiet` at 0.09, stone is `normal` at 0.21, and tile is `loud` at 0.90. Crouch uses a 0.75 source multiplier, standing movement 1.0, and sprint/landing 1.35. The slice guard keeps its 0.16 physical hearing floor and now uses a 0.30 footstep investigation source floor. Carpet stays below hearing even at sprint/landing strength; crouched stone is 0.1575 and stays below the physical hearing floor; standing/running stone can be heard nearby but remains mild-suspicion evidence; tile remains investigation-capable even while crouched. Engaged/searching state no longer lowers hearing or footstep interpretation floors;
...
**Automated:** pending current post-push CI — the Acoustics suite now expects exact 0.40/0.70/1.00 closed/half/open transmission while preserving the same topology, path-cost math, diagnostics, and failure behavior.

**Manual:** required — compare the same nearby sound with the door closed, half open, and open. Closed must remain clearly muffled but should no longer behave like near-total acoustic isolation; half-open should sit predictably between closed and open.
