...
**Done when:** weak hearing can create suspicion and decay; investigation-worthy hearing from unaware/search makes the guard stop, turn to the evidence, hold a resolved 0.80–2.00 s varied stare, and only then move at investigating speed; visual suspicion grows continuously at an exposure-dependent rate instead of instant confirmation, with high exposure filling faster than low exposure; the same pre-investigation stare occurs when visual suspicion first becomes worth investigating; confirmed pursuit uses running speed while unaware patrol uses walking speed; current suspicion/stare truth is saveable; loss/recovery/search still preserve local knowledge; and existing Phase 3/4/5 regressions remain compatible.
