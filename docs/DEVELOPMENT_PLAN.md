...
**Automated:** accepted on mapper-workflow implementation head `e0f0f6771b9b653da7f6685eedb277e4064b93d4` by GitHub Actions Test run #495. The Authoring suite proves the FGD exports real `opening_variant(choices)` / `prop_variant(choices)` fields with their intended defaults while `visual_model_path` remains an optional blank string override; the representative `.map` proof contains only semantic IDs + selected variant keys and no generated-scene/model-path/collision override; real FuncGodot construction resolves Narrow/Tall onto the existing production `OrdinaryDoor` / `OrdinaryProp` owners with the narrow/tall model defaults and tall collision dimensions; persistent identity validates; the read-only verifier leaves source unchanged; and the older explicit opening/prop model/collision override regressions remain green. The same run ended with `ALL AUTHORING TESTS PASSED`, `ALL APPLICATION TESTS PASSED`, and `ALL TEST SUITES PASSED`. Successful-run script-error signatures were identical to the preceding green run #494 and remain existing deliberate failed-restore fixture noise.
