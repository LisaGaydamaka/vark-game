...
**Automated:** fix-forward validation pending. The mapper acceptance pass exposed that `vark_prop.prop_id` was omitted from TrenchBroom because its FGD default was `StringName`. The first correction made only the FGD side a `String`; GitHub Actions run #497 then correctly failed because FuncGodot's assembler requires the FGD property type to match the generated scene property type and refused String → StringName auto-apply. The root correction therefore makes the mapper-facing/runtime storage `VarkOrdinaryProp.prop_id` an ordinary `String` as well. Its semantic content-ID API already returns `String`, so this changes no save identity or gameplay semantics while satisfying FuncGodot's exact type contract. The Authoring suite continues to require exported `prop_id(string)` and real imported IDs on standard/tall production props. A full audit of project-owned `authoring/fgd/*.tres` found no other mapper-facing `StringName` defaults.
