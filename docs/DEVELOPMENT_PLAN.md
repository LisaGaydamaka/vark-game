...
- `VarkOrdinaryProp` resolves mapper-authored external Mesh paths at runtime with the scene's default Mesh as fallback, localizes each instance's box shape before applying authored dimensions, and keeps model/collision configuration out of semantic save snapshots because the authored world rebuild owns configuration while semantic snapshots own runtime phase/transform/velocity.
...
**Automated:** pending exact-head post-push validation. The Props suite must retain all accepted Phase 3 carry/release/throw/support/impact/traversal/save seams and additionally prove model-path/collision-size variant configuration, real player support on a stack, and ordinary-door obstruction/recovery. The Authoring suite must build two real `vark_prop` entities through FuncGodot from mapper-style source and verify stable IDs plus distinct compatible model/collision configuration. Existing save/transient-state coverage remains authoritative for carried/moving prop restore.
