...
A development mission-package **Guard/Nav Lab** builds its structural floor/wall/offset doorway from the real FuncGodot `.map` source, then bakes a Godot `NavigationMesh` from the imported static collision geometry using the Godot 4.7 source-geometry → bake API. The authored A/B points lie on opposite sides of a wall while the only opening is deliberately offset, so a valid route must bend through imported architecture rather than moving directly between coordinates. The 1.30 m ordinary leaf closes exactly between the authored jambs with no side voids. When the route reaches the configured closed ordinary door, the guard stops, reads the existing `is_navigation_passage_open()` seam, sends the same `VarkOrdinaryDoor` an idempotent `request_open()` intent, and waits for its authoritative OPEN state. Temporary physical obstruction may latch door motion, but while the passage is still required the guard retries that OPEN intent; once the blocker leaves and the door opens, patrol resumes. Player `interact()` remains the ordinary toggle. There is no guard-specific door or second navigation truth.
