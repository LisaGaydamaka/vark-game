...
- `vark_gameplay_light` is a TrenchBroom point entity backed by the real `VarkGameplayLight` scene. It owns `persistent_id`, unique `gameplay_light_id`, shared author-facing `control_id`, initial on/off state, gameplay exposure strength, rendered OmniLight range/energy/color/shadows, a reusable imported fixture-asset scene, and optional direct extinguish interaction.
