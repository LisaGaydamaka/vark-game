...
The words are a billboarded world-space `Label3D` above the speaker with depth testing disabled. That means visual cover does not erase words the active utterance is currently acoustically carrying to the player: the acoustic graph remains the authority, while presentation is intentionally readable through cover. Opacity is a **continuous function** of the live propagated-strength margin above the listener's hearing threshold: it is exactly zero at/below threshold and uses a smoothstep-shaped fade whose slope eases to zero at the inaudible boundary. The same alpha drives both the Label3D text fill and its independent outline tint, so no opaque outline silhouette remains after the fill fades. Moving away during one line therefore fades and eventually hides the same words immediately; moving back into range before the utterance expires reveals them again without replaying the semantic event. When the presentation lifetime ends, the text is hidden regardless of position.

The lab uses the real player/session path plus **two structural acoustic spaces** separated by the cover wall. One full-transmission acoustic portal sits at the wall's open edge, so sound does not pass straight through the wall: listeners in the cover-side space must route speaker → opening portal → listener. **OPENING / LOUDER** and **BEHIND COVER / MUFFLED** are deliberately near the same straight-line distance from the speaker, but the behind-wall position has a longer portal route and therefore lower propagated strength. **NEAR / MARGINAL / INAUDIBLE** extend the same proof. The proof line uses a provisional 0.75 gameplay strength so the routed cover case remains readable while still fading to zero farther away.

**Done when:** a line cannot appear before its one semantic `gameplay.sound` event crosses the controlled acoustic consequence boundary; structural cover is represented by the same acoustic spaces/portals used by ordinary gameplay sound, so moving from an exposed opening position behind the wall lowers current propagated strength because the route must travel through/around the authored opening rather than because speech runs a private LOS rule; during the same active utterance the words continuously fade as current propagated strength falls, disappear at/below hearing threshold, and can reappear if the player returns to audible space before expiry, all without another semantic sound event; representative visual cover does not erase acoustically audible words; expiry clears stale text; the source text/lifetime comes through the tiny line-ID/data Resource rather than guard AI or hardcoded production dialogue; diagnostics distinguish semantic queued/heard counts from live presentation propagation; and no conversation tree, writer tooling, localization framework, voice playback authority, awareness behavior, generic intra-space obstacle occlusion, or Phase 12 dialogue system is pulled forward.

**Automated:** implemented — the Speech suite is wired into `tests/run_all_tests.gd`. It verifies Development Launch exposes Speech Lab, the proof text/lifetime comes from a valid `VarkSpeechLine` Resource, and the lab launches through Application → WorldSession → Player → the existing acoustic graph with exactly two structural spaces and one cover-edge portal. The opening and behind-cover positions are nearly equal in direct source distance, but a real vision ray is clear at the opening and blocked by the wall behind cover; both acoustic results use `portal.cover_edge`, and moving behind the wall during one active utterance must continuously increase route cost / lower propagated strength and whole-glyph opacity while queued/heard semantic counts remain unchanged. Continuing farther must keep fill/outline alpha equal, produce several small monotonic fade steps near threshold, hide at the inaudible marker, and reappear when returning to audible cover range without another semantic event. A short duplicate line also proves expiry removes the words.

**Manual:** pending corrective recheck — validator: **Windows x64 user/playtester**. Launch **Development Launch → Speech Lab**. First compare **OPENING / LOUDER** with **BEHIND COVER / MUFFLED**: they are intentionally at roughly the same direct distance from the speaker, but moving inward behind the wall during one utterance should continuously reduce the whole-glyph opacity because acoustics routes around the wall's open edge. The wall should visually hide the speaker at the cover marker while the still-audible words remain readable. Continue toward **MARGINAL** and **INAUDIBLE** during that same line: white fill and black outline must fade together, with the last part easing gradually to nothing. Move back toward the opening before expiry and the same line should strengthen again without a new semantic sound event. Judge whether the change feels like sound travelling around structural cover rather than simple distance-only fading.
