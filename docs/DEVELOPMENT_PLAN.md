...
**Current status:** implementation and deterministic regression coverage are checked in on `test`. The first asymmetric vertical-city playtest package is preserved unchanged as **Representative Stealth**. The separate **Representative Stealth — Ledge City** package is now revision 3: the street/cross-street/service alleys and interior corridors are substantially tighter; room shells use overlapping joints and exact-size ordinary-door openings to eliminate visible seam/gap artifacts; and traversal is disguised as normal city fabric rather than dedicated route platforms. Street carts/crates/stalls/canopies feed into attached awnings, balconies, window sills, cornices and fire escapes, then into genuinely tilted roofs with dormers/chimneys/parapets and more optional vertical choices. The west exterior high-window and east Watchmaker-key/Archive-interior approaches remain distinct. The guard-path connectivity gate/live-patrol regression remains in force, including removal of road-spanning decorative curbs that baked as walkable but physically blocked the guard body. Post-push CI is green for revision 3; keep this item `[~]` until focused user/playtester acceptance is successful.
