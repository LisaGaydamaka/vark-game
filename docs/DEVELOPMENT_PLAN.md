...
**Automated:** reopened for the reported top-supported-crate door regression. Existing run #413 remains the accepted baseline for the original 6.6 implementation; exact-head post-push validation for the support-motion correction is pending. The added Props regression must prove both sides of the contract: a crate in the leaf's side sweep still latches ordinary obstruction, while a crate genuinely supported on the leaf's top edge is released into the existing unsupported rigid-body path and does not prevent the same door from moving.
