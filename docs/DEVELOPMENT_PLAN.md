...
**Automated:** implemented — a new Objectives suite is wired into `tests/run_all_tests.gd`. It verifies Development Launch exposes Objective Lab, launches it through the real application/session/player path, observes `mission.completed` through the existing semantic event bus, proves the initial active/locked queries, proves unknown queries fail closed, inspects both world triggers as semantic-only emitters, physically moves the real player through EXIT before the objective and verifies the attempt is blocked, moves through OBJECTIVE and verifies the public queries update after the consequence pass, then re-enters EXIT and verifies exactly one completion event. A third exit entry proves completion is idempotent.

**Manual:** pending — validator: **Windows x64 user/playtester**. Launch **Development Launch → Objective Lab**. Walk to **EXIT** first; the status must remain **EXIT LOCKED** and the route must not complete. Walk through **OBJECTIVE**; status should change to **OBJECTIVE COMPLETE / EXIT UNLOCKED**. Return to **EXIT**; status should become **ROUTE COMPLETE**. Re-entering EXIT afterward should not visibly retrigger or duplicate completion. The intended feel is a minimal beginning/end contract, not a polished objective UI.
