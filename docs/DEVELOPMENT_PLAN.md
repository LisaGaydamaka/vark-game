...
- carpet is `quiet` at the near-silent 0.09, stone is `normal` at 0.45, tile is `loud` at 0.90, and crouch/sprint remain multiplicative gait modifiers on top of the selected surface tier. The real slice guard uses a second investigation gate for footsteps: a heard footstep must have at least 0.22 source strength as well as enough propagated strength. This makes stone sneak (0.2025) noticeable-but-suspicious-only even at point blank, while stone walk/run and tile sneak/walk/run remain investigation-capable. Carpet run peaks at 0.1215, below the slice guard's 0.14 hearing floor before attenuation, so all carpet movement is almost silent;
...
**Done when:** authored surface profiles can select only `quiet`, `normal`, or `loud`; canonical strength is derived from that tier rather than arbitrary per-surface tuning; the real Integrated Slice exposes carpet/stone/tile examples; carpet sneak/walk/run remain below ordinary hearing, stone sneak can create suspicion but cannot by itself trigger investigation at any distance, stone walk/run can investigate, tile sneak/walk/run can investigate, gait modifiers preserve the tier ordering, and all three route through the unchanged gameplay-sound/acoustic-listener path.

**Automated:** the authoritative Gameplay Noise suite checks the closed three-tier profile contract, real slice wiring, semantic sound routing, gait scaling, loudness-meter reporting, burst-movement cadence, and the hearing-only reaction matrix with awareness vision frozen: carpet remains below hearing for sneak/walk/run; close stone sneak is heard but repeated steps remain suspicious-only below the fixed 0.22 investigation source floor; stone walk/run investigate; tile sneak/walk/run investigate. The Phase 3 Integration suite also requires all three tiered surfaces in the production slice. Post-push CI must remain green.

**Manual:** required — while fully out of sight, verify the matrix directly: carpet should be almost silent for sneak, walk, and run; stone sneak may produce a mild suspicion tell when extremely close but must never make the guard investigate from that sound alone; stone walk/run should provoke investigation at useful close/medium ranges; tile should feel dangerous even while sneaking, with walk/run stronger still. Repeat stone sneak very close to the guard several times to ensure proximity or repeated weak steps do not cross the investigation ceiling. Acceptance should focus on NPC behavior, not the debug meter alone.
...
- `VarkGuard` keeps the narrow awareness-navigation override seam but locomotion now exposes exactly three semantic speeds from the existing feel values: walking/patrol 1.60 m/s (below the player crouch/sneak speed of 2.00), investigating/search 0.72 m/s (the previous slowest search speed), and running/pursuit 2.64 m/s (the previous fastest pursuit speed). Unaware patrol always uses walking; investigation and search cannot inject arbitrary fourth speeds through the motion-profile seam;
...
**Done when:** weak hearing can create suspicion and decay; investigation-worthy hearing from unaware/search makes the guard stop, turn to the evidence, hold a resolved 0.45–1.35 s varied stare, and only then move at investigating speed; visual suspicion grows continuously at an exposure-dependent rate instead of instant confirmation, with high exposure filling faster than low exposure; the same pre-investigation stare occurs when visual suspicion first becomes worth investigating; confirmed pursuit uses running speed while unaware patrol uses walking speed; current suspicion/stare truth is saveable; loss/recovery/search still preserve local knowledge; and existing Phase 3/4/5 regressions remain compatible.

**Automated:** the authoritative Awareness suite now additionally proves the production three-speed relationship (walk 1.60 < player sneak 2.00, investigate/search 0.72, run/pursuit 2.64), strong heard evidence creates a paused investigation stare before movement, the resolved stare duration/target/serial are semantic save truth, bright sight accumulates suspicion faster than the point-blank darkness override, neither path alerts on the first sample, visual evidence passes through the stare threshold before confirmation, and search-interrupt evidence also pauses/stares before reseeding. Existing pursuit/loss/search/save/door regressions remain the compatibility barrier.

**Manual:** required again because this pass intentionally changes perception timing and movement feel after prior acceptance. Verify that low exposure produces slow visible suspicion growth, high exposure fills materially faster without one-frame confirmation, an unaware/searching guard physically stops and faces investigation-worthy sight/sound before a varied stare, then moves slowly at investigating speed; unaware patrol is slower than player sneak; confirmed chase still uses the old fastest run. Report any case where the guard starts walking toward evidence before the stare ends or visually snaps from calm to alert.
...
- active search keeps ordinary senses live during movement, pauses, turns, and held looks; new evidence still interrupts the current action immediately. Vision now separates horizontal body facing from vertical attention: unaware guards use the normal vertical field, suspicion/investigation/recovery progressively widen it, and searching/alerted guards check steeply above and below while still requiring real range, LOS, and gameplay exposure. Elevated evidence keeps its true height, while search navigation accepts only reachable nav points and can also sample the guard's current floor when evidence is on another level;
