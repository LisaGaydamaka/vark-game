...
TrenchBroom gets two small point entities: `vark_acoustic_space` and `vark_acoustic_portal`. Space extents are authored in normal mapper units, not hidden Godot-meter values. FuncGodot scales origins but does not scale arbitrary entity properties, so the Vark acoustic-space boundary explicitly converts mapper extents using the current 32-units-per-world-meter contract; deterministic import coverage protects that assumption. The acoustic-space FGD also supplies a TrenchBroom-only hollow bounds display whose model scale is driven by those same half-extents (including the mapper↔Godot axis mapping), so the author can see the actual configured volume while editing without importing that display model as gameplay geometry. TrenchBroom 2026.2 treats a model-scale array as an ordered list of fallback scale candidates rather than XYZ components; the first attempted cage therefore collapsed to uniform X scaling. The corrected FGD emits one space-delimited vector string from the three entity properties, which is the non-uniform scale form TrenchBroom's model evaluator accepts. Portals author stable acoustic IDs, connected space IDs, optional ordinary-door ID, and closed/open transmission. This is the explicit mapper cost of the chosen prototype; no automatic room inference or geometry scanning is hidden behind the workflow.
...
**Done when:** one world-owned propagation owner consumes `gameplay.sound` only through the accepted session event route; same-space distance attenuation, connected portal routing, disconnected-space rejection, and multi-portal corner routing are deterministic; the real ordinary door continuously changes the same portal's transmission; representative footstep/speech/impact source strengths produce useful listener evidence without coupling to AudioStream volume; malformed duplicate/missing topology fails closed with useful diagnostics; TrenchBroom exports the two topology entities, the acoustic-space volume is visibly legible and tracks its authored half-extents in TrenchBroom, and real FuncGodot import proves normal mapper-unit extents convert correctly; the mapper cost is explicit; and no guard AI, awareness state, nav framework, presentation-audio authority, or generalized production acoustic framework is pulled forward.

**Automated:** required — the new Acoustics suite is wired into `tests/run_all_tests.gd`. It must prove FGD export including the property-scaled TrenchBroom acoustic-space bounds display and its model asset, authoritative 32:1 map-scale alignment, real FuncGodot topology import/reimport, mapper-unit extent conversion, duplicate/missing-link failure diagnostics, Acoustic Lab launch through the production application/session path, same-room attenuation, disconnected-room rejection, two-portal corner routing, continuous ordinary-door transmission, queued semantic-event timing, and representative thresholded footstep/speech/impact hearing relationships. The existing authoring/application/props/movement suites remain authoritative alongside it.

**Manual:** pending — validators: **Windows x64 user/playtester** for acoustic intuition and **Windows mapper/user with TrenchBroom 2026.2** for authoring cost. In **Development Launch → Acoustic Lab**, trigger the blue/green/red probes and compare listener labels with the open room, closed/open ordinary door, disconnected room, and around-corner route. The same source should become materially easier to hear through the real door as it opens; the disconnected room must stay silent; the corner route should feel plausible rather than like straight-line radius hearing. Confirm movement/look/interact remain normal. The first mapper pass found a real acceptance failure: acoustic-space properties were editable but the configured volume had no visible borders. The first visibility correction also failed manual acceptance because TrenchBroom interpreted the attempted numeric scale array as fallback candidates, making `mapper_half_extent_x` uniformly scale the whole cage while Y/Z did nothing. After refreshing the Vark TrenchBroom config, retest a disposable `vark_acoustic_space`: its hollow bounds cage must be visible, changing X/Y/Z independently must resize width/vertical height/horizontal depth independently, and save → close/reopen must retain the same non-uniform volume. Then place/edit a `vark_acoustic_portal`, save/reimport, and confirm the authored topology survives without generated-output hand edits. Exact attenuation/transmission tuning remains provisional until this playtest is accepted.
