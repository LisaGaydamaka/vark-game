...
**Automated:** implemented — a new Visibility suite is wired into `tests/run_all_tests.gd`. It launches Exposure Lab through the production application/session path, verifies the sources are real shadow-casting `OmniLight3D` nodes, exercises the real player capsule at all six labeled fixture positions, asserts deterministic dark < edge < partial < full relationships, proves opaque world geometry occludes all representative body samples, proves two lights contribute additively before 0–1 clamping, and verifies the development light-gem/debug HUD is driven by the live world-owned exposure summary. The numeric falloff/tuning remains provisional pending the manual visual-intuition pass.

**Manual:** pending — validator: **Windows x64 user/playtester**. Launch **Development Launch → Exposure Lab** and walk the labeled positions. The top-left light-gem/debug value should be almost dark at **DARK**, low but nonzero at **LIGHT EDGE**, clearly higher at **PARTIAL**, high at **FULL**, drop strongly at **OCCLUDED** behind the wall, and show a sensible combined increase at **TWO LIGHTS**. More important than exact numbers: the readout should broadly agree with what the lit space looks like while moving naturally across the boundaries. If the visual/readout relationship feels wrong, keep 3.8 open and tune this prototype rather than treating the current formula as a permanent contract.
