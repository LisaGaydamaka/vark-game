...
**Automated:** accepted on interaction-highlight implementation head `ebae8c261903e5a8524edfbb5017755c0a92d230` by GitHub Actions Test run #397. The Application suite proved an aimed interactable lamp makes both body and lit surface fullbright/unshaded with no received scene shadows while preserving the body albedo, non-emissive body material, and ordinary cast-shadow behavior; when interaction targeting ends, the exact authored body shading/receive-shadow state returns without color change. The previously accepted fixture self-fill, emitter/glass ON/OFF, exposure, save/load, TrenchBroom control-ID authoring, and full regression coverage all remained green. The run ended with `ALL APPLICATION TESTS PASSED` and `ALL TEST SUITES PASSED`. The Application suite proved the fixture keeps the accepted authored emitter position and solid collision, adds a private unshadowed render-only self-fill that illuminates only the normally shaded fixture body, and keeps that body on the ordinary render layer so other scene lights can still illuminate it. The ON/OFF regression proves the body material instance, albedo, and shading mode remain unchanged; only the designated glass/emitter state changes, while both the real world emitter and fixture-only self-fill go to zero when OFF. Existing gameplay exposure, save/load, TrenchBroom control-ID authoring, visibility, and all full regression suites remained green. The run ended with `ALL APPLICATION TESTS PASSED`, `ALL VISIBILITY TESTS PASSED`, and `ALL TEST SUITES PASSED`.
