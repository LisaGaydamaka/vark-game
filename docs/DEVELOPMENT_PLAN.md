...
A development-only **Integrated Slice** now forces the accepted Phase 3 implementations to coexist in one ugly playable route instead of remaining isolated labs. It uses the production Application → WorldSession → Player path and one compact two-room graybox separated by the real ordinary door. The north room adds a cover wall and one real shadow-casting gameplay light; the south room is the start/exit side. One real `VarkGuard` patrols between the rooms on a runtime-baked `NavigationMesh`, detours through the single doorway, and uses the same ordinary-door navigation seam proven in 3.7.
...
Two deliberately slice-local footstep surface probes make the route readable without pretending Phase 5 surface/audio work is already designed. The south **STONE / LOUD STEPS** area and north **CARPET / QUIET STEPS** area feed a tiny player-distance footstep emitter that queues ordinary semantic `gameplay.sound` facts as `footstep.stone` / `footstep.carpet` with different source strengths. It owns no presentation audio and is not a generalized material/surface framework.

NPC reaction is likewise thin integration glue rather than the future awareness system. A slice-local guard reaction adapter consumes the guard's real acoustic-listener evidence for `footstep.*` and `prop.impact`. The first heard player noise turns the guard toward the source, exposes a development reaction label, and asks the existing `VarkWorldSpeechSpeaker` to say one resource-authored **“What was that?”** line. That response is itself the normal semantic speech sound and only appears to the player through the existing live acoustic presentation path. The same adapter also provides a primitive visual reaction by combining the accepted world-owned player exposure scalar, guard facing/range, and an ordinary physical LOS ray; this is an integration probe only, not durable awareness/search/detection state.
...
**Done when:** Integrated Slice launches from Development Launch through the normal application/session/player path; runtime navigation is nonempty and the real guard moves/uses the same door; the guard remains registry-addressable by the accepted persistent/content identity; the acoustic graph contains the two spaces, one door portal, player listener, and guard listener; the same door materially changes propagated strength across the rooms; loud-stone and quiet-carpet surfaces emit semantic footstep strengths without presentation audio; one loud real player step can be heard by the guard and cause exactly one resource-backed world-space spoken reaction that is itself acoustically gated for the player; real Junk can still be picked up/thrown in the same world; the existing exposure owner plus LOS can produce the slice-only primitive visual reaction; the objective/exit contract still blocks early exit, unlocks after objective, and completes afterward; and none of those integrations introduce a replacement door, sound, nav, actor identity, speech, exposure, prop, or objective truth.

**Automated:** implemented — a new **Phase 3 Integration** suite is wired into `tests/run_all_tests.gd`. It launches the slice through Application, verifies the real component set plus stable guard registry identity, checks nonempty nav and guard door use, verifies the shared door changes the shared portal's propagated strength by a large margin, emits a real loud-stone footstep through the slice emitter and requires the guard's real listener/reaction plus one world-space speech response, exercises the accepted player Junk carry→throw path in the same world, places the player in the lit north room and requires exposure+LOS to drive the slice-only vision reaction, then proves the early-exit/objective/final-exit semantic route still works.

**Manual:** pending — validator: **Windows x64 user/playtester**. Launch **Development Launch → Integrated Slice** and play the route as a small stealth sandbox rather than following one exact script. Confirm: ordinary movement/look still feel normal; **F** opens/closes the same door and picks up/throws Junk while **R** gently releases it; the guard patrols through the doorway without clipping/stalling and may reopen a genuinely blocking door; walking on **STONE / LOUD STEPS** is materially more likely to make the guard show **HEARD NOISE** / say **“What was that?”** than the quieter **CARPET / QUIET STEPS**, and door position/room separation should make that hearing feel spatially plausible; the typed line stays attached to the guard and follows current acoustics; throwing a crate can create the same kind of hearing reaction; the top-left exposure readout rises in the north light and physical cover/darkness can keep it lower, while an exposed clear-LOS approach can show **PLAYER SEEN**; reach **OBJECTIVE**, then return to **EXIT** and get **ROUTE COMPLETE**. Report any contradiction such as door/nav fights, impossible hearing, speech that ignores current acoustics, prop/carry failure, light-gem mismatch, objective/exit failure, guard stalls/clipping, or ordinary movement regression. Final stealth tuning, reaction persistence/search behavior, presentation audio, and combat are explicitly not being judged yet.

**Phase gate:** pending the manual integrated-slice pass — the dangerous assumptions have isolated proofs, durable semantic mutation/event/lifecycle timing exposes one stable gameplay boundary, actor identity survives life-state changes, and this route now exercises the same implementations together. Phase 3 closes only after the integrated route is manually accepted without an architectural contradiction.
