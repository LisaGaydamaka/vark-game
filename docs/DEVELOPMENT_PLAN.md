...
**Done when:** Integrated Slice launches from Development Launch through the normal application/session/player path; its only rendered shadow light is a `VarkGameplayLight` sampled by the exposure owner; runtime navigation is nonempty and the real guard moves/uses the same door; the guard remains registry-addressable by the accepted persistent/content identity; the acoustic graph contains the two spaces, one door portal, player listener, and guard listener; the same door materially changes propagated strength across the rooms; loud-stone and quiet-carpet surfaces emit semantic footstep strengths without presentation audio; fully crouched footsteps are semantically quieter than standing footsteps while preserving their surface identity; one loud real player step can be heard by the guard and cause exactly one resource-backed world-space spoken reaction that is itself acoustically gated for the player; real Junk can still be picked up/thrown in the same world; the existing exposure owner plus live crouch geometry/LOS can produce standing-visible versus crouched-behind-low-cover behavior without a fixed-height sight target; the objective/exit contract still blocks early exit, unlocks after objective, and completes afterward; and none of those integrations introduce a replacement door, sound, nav, actor identity, speech, exposure, prop, movement, or objective truth.

**Automated:** corrective coverage implemented — the **Phase 3 Integration** suite remains wired into `tests/run_all_tests.gd`. In addition to the original component/nav/door/acoustics/speech/Junk/exposure/objective checks, it now requires exactly one visible rendered `Light3D` in the slice and requires that node to be the real shadow-casting `VarkGameplayLight`; verifies a standing stone step emits its full `0.52` semantic strength and is heard in the representative cross-room open-door case; fully crouches the real player capsule/head and verifies the same stone surface emits `0.52 × 0.45` and becomes muted in that same acoustic case; then places guard/player across **CROUCH COVER**, proves standing remains gameplay-lit and visible, fully crouches the same player, proves exposure is still above the visual threshold while the live head target drops by more than 0.40 m, and requires the physical LOS blocker to be exactly `CrouchCover`.

**Manual:** pending corrective recheck — validator: **Windows x64 user/playtester**. Launch **Development Launch → Integrated Slice** and play the route as a small stealth sandbox. First verify the lighting contract: there should be no global/directional shadow source painting misleading drop shadows; the north shadow-casting light should be the same light represented by the top-left exposure readout. Use **C** to crouch. On the same surface and at comparable distance, crouched movement should be materially less likely to trigger **HEARD NOISE** than standing movement, while **STONE** remains louder than **CARPET**. At **CROUCH COVER**, approach the guard/light side while standing so **PLAYER SEEN** can occur, then crouch behind the low cover: your camera/body should physically lower and the current visual reaction should clear because the guard's LOS now targets the lowered live head, even if the exposure gem still says you are lit. Continue checking the existing route: **F** opens/closes the same door and picks up/throws Junk, **R** gently releases it; the guard patrols through the doorway without clipping/stalling; door position/room separation affect hearing plausibly; the typed line follows current acoustics; thrown Junk can create hearing reaction; reach **OBJECTIVE**, return to **EXIT**, and get **ROUTE COMPLETE**. Report any remaining visual-light/exposure contradiction, crouch that does not materially affect hearing/cover visibility, door/nav fight, impossible hearing, speech/acoustic mismatch, prop/carry failure, objective/exit failure, guard stall/clipping, or ordinary movement regression. Final stealth thresholds, search behavior, presentation audio, combat, and art are explicitly not being judged yet.

**Phase gate:** pending the corrected manual integrated-slice pass — the dangerous assumptions have isolated proofs, durable semantic mutation/event/lifecycle timing exposes one stable gameplay boundary, actor identity survives life-state changes, and this route exercises the same implementations together. Phase 3 closes only after the integrated route is manually accepted without an architectural contradiction.
