...
**Current status:** implementation and deterministic regression coverage are checked in on `test`. The current playtest revision replaces the flat/dense-compound layout with an asymmetric vertical city block: streets and alleys between irregular building masses, 100 independently authored world brushes, mandatory low street mantles, scattered stalls/stoops/awnings/ledges, a raised west window feeding a multi-stage rooftop route above 9 m, an elevated east key balcony with a separate descent to the locked street door, repeated north-street barriers, and a roof descent into the archive quarter. Existing light/import/hotkey corrections remain in place, and post-push CI is green for the revised production build/navigation/save path. Keep this item `[~]` until the revised two-route user/playtester acceptance is successful.
