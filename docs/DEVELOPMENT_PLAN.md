...
**Automated:** implemented through new authoritative `tests/awareness/run_awareness_tests.gd`, added to `tests/run_all_tests.gd`. It launches the real Integrated Slice, shortens only test durations, freezes patrol motion while leaving awareness processing live, exercises weak/strong local hearing, gameplay-time decay, investigation/search navigation ownership, save/quickload of resolved search truth, confirmed visual pursuit, alert-loss grace, last-seen search, recovery, and return to patrol. Existing Application/Integration hostile and restore tests keep their legacy debug assertions through the compatibility view while the new suite asserts authoritative `awareness_state`. The 5.4 manual pass also exposed a pre-existing door/nav deadlock during awareness-driven movement: a fully open leaf could physically block a side approach, causing the guard to retry the already-satisfied OPEN intent forever. A first recovery implementation then proved that leaf-contact prediction alone was too broad and could close/reopen doors that the guard was merely moving beside. The bounded fix-forward now makes the **fixed doorway crossing** authoritative: the remaining NavigationAgent route must change sides through the authored opening width before any door OPEN/CLOSE behavior is relevant. Only after that intent exists may current-leaf collision trigger the geometry-derived same-side operating / close → reopen / centered-cross recovery, and the original patrol/awareness target resumes after doorway clearance. The Navigation suite protects both the required-crossing recovery and the same-side-near-leaf non-trigger without introducing general dynamic avoidance or completing the broader 5.6 door/nav/perception item.

**Manual:** required before accepting any later tuning that changes suspicion/alert/search timing or stealth predictability. This first 5.4 implementation introduces new player-facing guard behavior, so after automated acceptance the user must run one focused Integrated Slice playtest before 5.4 can be marked `[x]`: verify that a loud noise causes investigation/search, clear sight causes pursuit, breaking sight causes a believable short search rather than instant forgetting, and the guard eventually returns to patrol without feeling stuck or omniscient. The rerun must also reproduce the reported open-leaf side-approach case and confirm the guard performs the bounded close/reposition/reopen maneuver only when its route actually crosses that doorway; moving/searching/pursuing beside an open leaf without crossing the doorway must not make the guard touch the door.
