...
- `VarkGameplayLight.set_enabled_state()` never hides the fixture object. ON/OFF drives three coordinated pieces of one semantic state: semantic gameplay exposure, actual emitter energy, and the fixture asset's authored lit-surface appearance. ON restores emitter energy and bright/emissive glass/flame surfaces; OFF sets emitter energy to zero and changes those surfaces to their authored dark/hidden-off appearance while the body remains visible. It emits detached `light.state_changed` facts. `vark_switch` uses that same operation and emits `switch.used`; direct extinguish uses the same operation with source `direct`.
