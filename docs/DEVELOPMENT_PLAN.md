...
**Current status:** implementation and deterministic regression coverage are checked in on `test`. The first asymmetric vertical-city playtest package is preserved unchanged as **Representative Stealth**. The separate **Representative Stealth — Ledge City** package is now revision 2 and rebuilds the traversal stress test around a believable organized boulevard: continuous main street/cross street/service alleys, accessible hollow buildings with front doors and rooms, a rooftop Watchmaker key room, and a tall Archive with a locked street entrance, internal switchback stairwell/mezzanines and high-window exterior bypass. Jump→catch/hang→mantle and ledge-to-ledge transfers remain required, but they are expressed through awnings, balconies, fire escapes, cornices and building roofs instead of disconnected climbing towers. The previously observed guard no-route failure is covered at the root: the shared representative navigation gate now requires a real path between authored patrol endpoints, and the focused regression requires a live patrol leg on the clear boulevard. Post-push CI is green for the rebuilt package and guard-route correction; keep this item `[~]` until focused user/playtester acceptance of the rebuilt variant is successful.
