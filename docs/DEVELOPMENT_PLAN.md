...
**Automated:** implemented — a new Actors suite launches the accepted Guard/Nav mission through Application → WorldSession, resolves the authored guard through both persistent and content registries, proves conscious navigation movement, observes `actor.life_state_changed`, checks pre-drain and post-drain unconscious/dead states, confirms the same actor node and `NavigationAgent3D` survive both transitions, proves non-conscious locomotion stops while an external body-style reposition preserves identity, verifies resurrection requests fail, checks detached capture semantics, and stops the session to verify quiet restore-style unconscious/dead application plus wrong-identity rejection. Navigation authoring regressions also require the guard FGD/source to carry the new persistent/content IDs.

**Manual:** none required — this roadmap item is an architecture/identity/timing compatibility proof with deterministic acceptance and no new player-facing combat, knockout interaction, dragging/ragdoll behavior, tuning, or subjective presentation decision to validate. The visible mesh tilt/color is diagnostic derived presentation only and is not a production behavior contract.
