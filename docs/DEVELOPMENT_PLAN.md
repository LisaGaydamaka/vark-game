...
**Automated:** pending exact-head post-push validation. The Authoring suite must prove `vark_breakable` mapper construction. The Application suite must prove effect filtering, real thrown-prop impact delivery, ordinary-door nonresponse, one-shot event semantics, and persistent broken-state capture/restore without replay.
