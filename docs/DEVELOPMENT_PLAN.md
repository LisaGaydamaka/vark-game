...
- carpet is `quiet` at the near-silent 0.09, stone is `normal` at 0.45, tile is `loud` at 0.90, and crouch/sprint remain multiplicative gait modifiers on top of the selected surface tier. This adds the missing loud tier without changing the accepted carpet/stone baseline.
...
**Done when:** authored surface profiles can select only `quiet`, `normal`, or `loud`; canonical strength is derived from that tier rather than arbitrary per-surface tuning; the real Integrated Slice exposes carpet/stone/tile examples; standing carpet emits quiet `footstep.carpet` at 0.09, stone emits normal `footstep.stone` at 0.45, tile emits loud `footstep.tile` at 0.90, gait modifiers preserve the tier ordering, and all three route through the unchanged gameplay-sound/acoustic-listener path even when a propagated carpet step remains below hearing threshold.
...
**Manual:** required — user/playtester crosses the labeled carpet, stone, and tile regions at the same standing walking gait and verifies the intended extreme gradient: ordinary carpet walking should be almost impossible for a guard to hear unless extremely close, stone should remain the normal baseline, and tile should be clearly dangerous. Then crouch on carpet (effectively silent in ordinary play) and sprint on carpet (still very quiet, but visibly louder on the semantic meter than walking) to confirm gait still matters without making carpet a normal-risk surface. Acceptance should focus on stealth readability rather than the debug meter alone.
