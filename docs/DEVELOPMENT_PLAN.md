...
**Automated:** the previously accepted 6.5 semantic/control-ID/save behavior is reopened only for fixture ON/OFF presentation. Exact-head validation must additionally prove OFF keeps the imported fixture model visible, changes its designated lit surface to non-emissive dark/hidden-off presentation, sets actual emitter energy to zero, and quickload restores that same dark-visible state. The authoring suite proved the exported Vark TrenchBroom FGD exposes `vark_gameplay_light` and `vark_switch` linked only by `control_id`, and a real FuncGodot build created two saved gameplay lights plus one switch from mapper source without Godot-side node-path wiring. The Application suite proved one F toggles every light in the group, derived lever presentation follows the group, real gameplay exposure changes, direct extinguish changes the same persistent light truth, quicksave stores only light state, deliberate post-save divergence is possible, and quickload restores lights plus derived switch pose without event replay. The run ended with `ALL AUTHORING TESTS PASSED`, `ALL APPLICATION TESTS PASSED`, and `ALL TEST SUITES PASSED`.
