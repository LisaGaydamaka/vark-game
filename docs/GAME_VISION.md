...
While actively investigating or searching, the NPC is deliberately more attentive because prior evidence has established that something may be wrong. In the default Thief-like tuning this primarily strengthens visual interpretation: useful visual evidence begins somewhat lower and continuous visual suspicion grows somewhat faster. Ordinary hearing thresholds and footstep source floors do not become more permissive merely because the guard is engaged; persistence comes from the awareness/search state itself. This heightened attention does not extend acoustic/vision reach, bypass occlusion, or reveal the hidden current player position.
...
Ordinary lamps, street lamps, candles, torches, and similar light-bearing objects remain present as world objects when switched off or extinguished. A reusable light-fixture asset separates the always-visible imported body from designated lit-surface geometry. ON makes those surfaces use their authored bright/emissive state and enables the actual light emitter; OFF disables emitter energy and changes those surfaces to their authored dark or hidden-off state without hiding the fixture body. A candle asset may hide only its flame surface when off; a street-lamp asset may keep its glass visible but dark. Gameplay exposure, emitter state, lit-surface state, and save/load all derive from the same semantic light state. The fixture body's imported/authored materials are not part of that ON/OFF state: only designated glass/flame/bulb surfaces may change state-driven color/emission.

## Light fixture emitter/collision ownership — LOCKED

The mapper origin places the light fixture object; it is not assumed to be the physical light-emission point. Every ordinary light-bearing fixture asset owns an explicit `EmitterAnchor` positioned inside its luminous glass/flame/bulb region, and `VarkGameplayLight` places its child light emitter and gameplay-exposure origin at that anchor. The fixture also owns solid authored collision that remains active in both ON and OFF states, so lamps and similar physical fixtures block the player. The coarse player-solid fixture collision is excluded from its own emitter-to-player exposure rays because it encloses the luminous glass, but opaque fixture metal must still block stealth exposure. Each fixture therefore owns a private exposure-only static occluder generated from its opaque body mesh; gameplay-light rays include that private layer so metal/frame geometry blocks semantic exposure in the same places it casts rendered shadows while the glass volume itself does not falsely self-occlude. Other world geometry still occludes normally. Because a shadow-casting emitter physically inside an opaque fixture can self-shadow the fixture itself, the asset may also contain a small unshadowed **visual-only self-fill** at the same emitter anchor. That helper affects only a private render layer on the fixture body, never ordinary world geometry, never gameplay exposure, and never replaces normal scene-light shading; the body stays on the ordinary render layer as well so other lights continue to illuminate it normally.
