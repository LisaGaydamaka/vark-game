...
`tests/noise/run_noise_tests.gd` is an authoritative top-level suite. It validates the closed three-tier profile contract and fail-closed invalid IDs/tiers, reusable surface/profile composition, then launches the real Integrated Slice through Application/WorldSession. It verifies carpet = quiet/0.09, stone = normal/0.35, tile = loud/0.90, all three use derived `footstep.<surface>` kinds, carpet run remains below the real slice guard's 0.16 hearing floor, point-blank crouched stone remains below hearing at 0.1575, nearby standing/running stone is above the 0.30 footstep investigation floor and can trigger investigation, and tile sneak/walk/run investigate. The suite also preserves loudness-meter truth, gait multipliers, and the grounded burst-movement cadence regression. `tests/integration/run_phase3_slice_tests.gd` also requires all three profile-driven surfaces in the production slice wiring.

The existing Phase 3 Integration suite continues to protect the accepted cross-system route, but now reads the surface strength from the profile-derived semantic summary. Prop impacts, doors, speech, and hostile-impact sounds remain direct users of the same three-field `gameplay.sound` contract. Current stealth tuning raises the production guard listener floor from 0.14 to 0.16 in both Integrated Slice and Vertical Stealth Lab; this changes only physical audibility reach, not semantic source strengths, propagation, or the separate awareness interpretation thresholds.
...
`VarkPlayerFootstepEmitter` treats a normal airborne → grounded transition as one semantic landing sound on the current `VarkSurfaceProfile`. Landing uses exactly the same surface-relative multiplier as sprint/running (currently 1.35× base), records `last_gait = "landing"`, and replaces rather than stacks with a same-frame cadence footstep. Mantling has an explicit contact handoff: while traversal is `mantling`, cadence is suppressed and a pending mantle contact is remembered; successful grounded completion emits exactly one ordinary `walking` step when standing or `crouched` step at crouched strength when crouched. If mantle ownership ends while still airborne, the pending mantle contact is discarded, ordinary landing arming resumes, and the eventual impact remains running-strength. The Gameplay Noise suite deterministically proves all three transitions—ordinary jump/fall landing, successful standing/crouched mantle completion, and cancelled mantle → airborne → hard landing—through the same helper used by the production physics process.

## Vertical Stealth Lab

Development Launch → **Vertical Stealth Lab** loads `missions/vertical_stealth_lab/world.tscn`, a larger manual stealth fixture built from the same real player, guard-awareness, acoustics, exposure, surface-noise, ordinary-door, prop, objective, and runtime navigation seams as the Integrated Slice. Its floor is 24×28 m, the west side provides four ascending climb blocks into an upper catwalk and 4.2 m objective tower, the east side adds a separate raised ledge, and the three surface volumes extend vertically so elevated movement/landings still resolve carpet/stone/tile gameplay-noise semantics. Six real gameplay lights cover the north/tower, south, center, west climbing route, and east patrol spaces; no decorative shadow light is used to fake stealth illumination. The entire 24×28 floor is partitioned continuously into exactly three 8×28 gameplay/visual regions with no neutral separator strips: west stone is blue-gray, center carpet is dark red, and east tile is pale gray. The generic dark-gray base-floor renderer is removed. Every rendered Geometry mesh now uses one of those same three materials: west structural planes are stone, center planes are carpet, east planes are tile, and wide divider/end-wall meshes are split exactly at the x=-4/+4 surface boundaries. Elevated walkable geometry follows the same rule (west climb/catwalk/tower stone, east ledge tile, low center covers carpet). The visual regions therefore never fall back to an ambiguous gray plane and remain aligned with the surface identities used by footstep/landing noise. The guard patrol endpoints are 20 m apart and use authored 2.5 s / 4.0 s dwell times.
...
All awareness durations are remaining gameplay-simulation seconds advanced from `WorldSession.gameplay_time_seconds`. Production keeps continuous sight and the source-facing stare architecture while using Thief-like tuning: base visual range 10.0 m; two stare freezes per rapid cue chain with a 12.0 s reset; engaged hearing and footstep floors stay at 1.00× calm; visual interpretation uses a 0.90× exposure floor and 1.25× suspicion growth; baseline visual growth is 0.10–2.00/s, decay 0.08/s, 0.40 starts investigation/hold, and 1.0 confirms alert. Suspicion lasts 4 s, investigation 12 s, search 25 s, lost pursuit 30 s, and recovery 8 s. Confirmed ALERTED pursuit remains exempt from distraction holds.

The awareness semantic snapshot now also stores visual suspicion plus the current resolved pre-investigation stare active flag, duration/remaining time, target, and serial. Restore reapplies the same navigation target and re-pauses/reorients an in-progress stare before play resumes, so random observation time is never rerolled. The old debug `state` key remains only for compatibility; new work reads `awareness_state` plus the explicit suspicion/stare fields.

`tests/awareness/run_awareness_tests.gd` is an authoritative top-level suite using the real Integrated Slice. It proves the production 10.0 m base vision range and Thief-like persistence constants, weak heard suspicion immediately stops/orients without creating a route, suspicious hold state is snapshot truth, two rapid investigation-worthy cues may use the hold but a third cannot restart it and instead commits immediately to navigation, the allowance replenishes after quiet gameplay time, the exact production 1.50–3.50 s hold and locomotion relationship (walking 1.20 < player sneak 2.00; investigating/search 0.72; pursuit run 2.64), visual suspicion can decay while the hold remains active, continuous bright-vs-dark suspicion pacing, a dedicated non-default fixture still exercises configurable heightened hearing while production engaged hearing floors remain neutral, confirmed visible/lost pursuit remains exempt from distraction holds, deterministic search/save truth, pursuit/loss/recovery, and vertical reacquisition. The suite shortens only timing values needed for CI. Gameplay Noise continues proving repeated stone sneak from a calm/suspicious baseline remains capped below investigation; the Awareness suite separately proves prior stronger evidence can heighten interpretation without changing the surface emission or acoustic listener. Compatibility fixtures follow the same semantics.

Phase 5.4 manual acceptance is complete. On tuned `test` head `7b0002a808a9cd2ce5a999eac1f0d286781095b7`, the user/playtester accepted the current perception feel as part of the Phase 5 stealth playtest, including immediate weak-cue stop/orient behavior, 1.50–3.50 s source-facing holds, bounded repeat observation patience, current visual range/exposure behavior, suspicion decay during a hold, vertical awareness, and the confirmed/lost-pursuit exemption from distraction holds.
...
`VarkGuard.resolve_local_search_points(anchor, radius, count, variation_key, excluded_points, minimum_separation)` samples/projects candidates on the synchronized navigation map and scores them only from evidence proximity, vertical agreement with the evidence, separation from already searched positions, route/path cost, and a stable deterministic variation term. Production defaults now begin at a 2.8 m radius, expand to 6.0 m, and request at least 1.4 m horizontal separation between search stops when the reachable nav space permits. Elevated evidence is sampled at its real height plus a guard-floor fallback; endpoint validation includes vertical agreement so a same-XZ point on an unreachable floor cannot masquerade as reachable. The variation key is derived from stable semantic evidence inputs; the resolved candidate array is then saved directly. Search stages expand radius only after exhausting their current candidates, lowering confidence as uncertainty grows. New heard/seen evidence recenters the process at high confidence rather than extending stale knowledge.

Search execution keeps its semantic cadence layered over the spatial planner, but locomotion now exposes only three actual modes: walking patrol, fixed investigating/search speed, and running pursuit. The Integrated Slice values are 1.20 / 0.72 / 2.64 m/s respectively, with walking deliberately well below the player crouch/sneak speed of 2.00 m/s while investigating and pursuit retain their previously accepted absolute speeds. The old per-leg search scale remains only as saved compatibility/diagnostic data; `VarkGuard` normalizes search motion-profile requests back to the investigating mode. At a search destination the navigation goal remains intact but locomotion is paused. The guard resolves an arrival pause, one-to-three directed look turns biased around the known evidence direction, held gazes, pauses between looks, and a departure pause. Timings, look count, direction, and per-leg speed come from the stable search seed/action serial. The previous sinusoidal in-place sweep is removed. Ordinary hearing/vision remain live during every action. While investigating/searching, production attention keeps hearing/footstep floors at 1.00× and modestly changes only visual interpretation (0.90× exposure floor, 1.25× visual suspicion growth); it never alters propagation, LOS, range, or hidden knowledge. New investigation-worthy evidence interrupts into the longer paused source-facing hold first; only after that resolved hold does investigating navigation begin. Vision treats horizontal body facing and vertical attention separately: calm guards keep a normal vertical field, suspicion/recovery widen it, ordinary investigation travel uses an intermediate field, and searching/alerted guards use the widest up/down attention while range, physics LOS, and gameplay exposure remain mandatory. An active source-facing investigation stare also retains the widest field until the hold completes, preventing a search-to-investigation state transition from dropping the very elevated evidence that triggered the stare. Vision now checks head/center/lower-body samples so one clipped head ray does not erase otherwise clear contact.

Confirmed pursuit is now explicit semantic state inside ALERTED. Current sight stores last confirmed world position, observed velocity, attention direction, and a resolved reachable pursuit approach. Alerted retention uses the last explicit target direction plus slightly wider facing/lower exposure retention thresholds, so path steering, edge-of-FOV motion, darkness flicker, and brief occlusion do not themselves mean the guard forgot the player. A missed sample enters `contact_grace`; expiry advances to `checking_last_known` while navigation remains fast pursuit. `VarkGuard.resolve_pursuit_approach()` projects only the observed position plus a short clamped observed-velocity continuation onto reachable navigation, including a guard-floor fallback for elevated/off-nav targets. Search begins only after the guard reaches/checks that approach or a bounded lost-pursuit timeout expires. While contact is visible, unrelated heard distractions do not rewrite pursuit truth; after contact is genuinely lost, sufficiently strong local evidence may update the last useful pursuit trail. All resolved pursuit fields persist in the awareness semantic snapshot.
...
The awareness semantic snapshot now stores the resolved candidate order plus stable seed, stage, uncertainty radius, confidence, search age, visited positions, current search action, action duration/remaining time, look directions/count, resolved movement-speed scale, residual alertness, pursuit mode, reachable pursuit goal, last confirmed velocity, attention direction, lost duration, and last-known check duration. The authoritative Awareness suite proves hidden-player motion cannot change resolved search truth, confidence/uncertainty progression is simulation-time based, production radius/separation defaults are widened, resolved stops honor configured horizontal spacing, search locomotion is slower than patrol while pursuit is faster, current pursuit mode/goal survive quicksave/quickload, a visible confirmed chase ignores unrelated distraction navigation, first visual loss remains ALERTED through contact grace, grace expiry keeps fast pursuit while checking the reachable last-known approach, strong local evidence can refresh only an already-lost trail, search starts from the latest explicit evidence after pursuit failure, stationary search actions actually pause navigation motion, quickload restores already-resolved semantic choices instead of rerolling, elevated visible lateral movement updates a reachable floor/route pursuit approach without dropping alert, steep elevated-player vision is rejected in unaware state but can be reacquired during widened search/alert vertical attention, and residual alert still decays/re-alerts correctly. The user/playtester accepted the current cadence, patience, pursuit persistence, held-look presentation, speed relationships, non-omniscient search behavior, and vertical-attention feel on tuned `test` head `7b0002a808a9cd2ce5a999eac1f0d286781095b7` as part of the Phase 5 stealth playtest.

## Phase 5 stealth playtest acceptance

The user/playtester accepted the current Phase 5 stealth feel on tuned `test` head `7b0002a808a9cd2ce5a999eac1f0d286781095b7`. GitHub Actions run #350 on that exact head completed successfully before reconciliation. This closes the required manual acceptance for 5.1 gameplay noise, 5.2 acoustic door transmission, 5.4 perception/awareness, and 5.5 pursuit/search. The accepted evidence includes surface/noise readability and landing/mantle contacts; closed/half/open acoustic ordering; current vision/hearing, suspicion, stare and pursuit behavior; and long local-evidence search/recovery including vertical cases. This acceptance records the current tuned values as the Phase 5 baseline; it does not authorize later work to restore older tuning values.

## Phase 5.6 NPC communication/local knowledge

`gameplay/npc/guard_communication.gd` owns explicit guard-to-guard knowledge transfer without changing the accepted perception/search tuning. A guard awareness owner emits `local_alert_confirmed(evidence_position)` only on a fresh transition into locally confirmed ALERTED sight. Its communication component queues one `npc.local_warning` report with reporter/faction/origin/evidence/strength and separately emits `npc.warning` as ordinary gameplay-significant sound containing only origin/strength. Receivers independently gate the report by consciousness, matching faction, and the existing `VarkAcousticPropagation.evaluate()` result at their real listener position and hearing threshold. Thus semantic event delivery may be world-wide while **knowledge acceptance remains local**.

Accepted local warnings call the existing awareness owner through `receive_shared_evidence(&"warning", ...)` and enter investigation at the reported point. `VarkAlarmChannel` supplies the deliberate broader path: an explicit mission/system caller raises one named alarm for an authored audience faction; only communication components subscribed to that alarm ID accept it, and they enter search around the alarm's reported evidence point rather than confirmed pursuit. Ordinary player confirmation does not auto-raise the authored alarm. An already ALERTED guard refuses shared evidence so second-hand information cannot replace stronger locally confirmed pursuit truth.

The alarm channel is a semantic save owner. Its active flag, last evidence position, source actor, and raise serial restore without re-emitting `npc.alarm_raised`; recipient investigation/search state is already persisted by guard awareness. This directly exercises the foundation restore-suppression rule for alarm propagation.

Development Launch → **Communication Lab** is a two-real-guard fixture cloned from the accepted Integrated Slice composition. The reporter and receiver use the same ordinary door/acoustic spaces, real acoustic listeners, real awareness and navigation, two explicit faction communication nodes, and one `building.main` alarm channel. The receiver has its own persistent/content identity and patrol route; the lab controller configures it against the same runtime-baked navigation map.

`tests/communication/run_communication_tests.gd` is an authoritative suite wired into `tests/run_all_tests.gd`. It proves the lab launches/configures, a confirmed local alert emits exactly one five-field warning report without current-player data, a closed-door weak route and wrong faction reject knowledge, an acoustically audible same-faction warning transfers only its reported evidence position, later hidden player movement cannot update that second-hand target, unsubscribed alarms are ignored, an explicit subscribed alarm starts search from its evidence anchor, and quicksave/quickload restores both active alarm state and recipient search with an empty semantic event queue and fresh communication counters (no restore-time replay). Exact implementation head `8aa547217c2fe37f7804a8e4ad32064895277146` passed GitHub Actions run #352 with `ALL COMMUNICATION TESTS PASSED` and `ALL TEST SUITES PASSED`. No subjective manual acceptance is required for 5.6's bounded architecture behavior.

## Phase 5.7 same-door navigation/perception integration

The accepted ordinary-door architecture already had dedicated proofs for interaction/collision/vision obstruction, navigation links/NPC use, acoustic transmission, and semantic restore. Phase 5.7 adds the missing **same-instance cross-system proof** on the real Integrated Slice instead of creating another door fixture or parallel door state.

`IntegratedSlice.get_door_integration_debug_summary()` is a read-only diagnostic join over the existing owners. It reports `slice.door` / `door.slice`, semantic phase/open fraction, the door's navigation-passage seam and link summary, guard door-use/traversal state, door acoustic openness plus `portal.slice.door` debug state, and the awareness owner's latest LOS blocked/blocker/distance sample. The method computes no gameplay truth and stores no duplicate state.

`tests/integration/run_door_nav_perception_tests.gd` is wired into `tests/run_all_tests.gd` as **Door/Nav/Perception Integration**. It launches the production Integrated Slice and first waits for the real patrol guard to use the door-owned navigation link. It then freezes only locomotion/automatic awareness for a controlled comparison across the actual doorway: applying CLOSED must make navigation passage false, acoustic openness 0, portal door openness 0, production guard LOS blocked specifically by `OrdinaryDoor`, and produce the muffled acoustic route; applying OPEN must make passage true, acoustic/portal openness 1, clear straight-through doorway LOS, and increase propagated strength by more than 2× on the same `portal.slice.door` route. Opening the doorway does not make the leaf transparent: the suite derives positions from the real rotated `CollisionShape3D` and proves production guard LOS is blocked by `OrdinaryDoor` across the fully open leaf with both actors in the same north room, then repeats the proof at a 0.5 partial opening. Door application regressions separately ray-test the fully open and partial physical leaf. Guard and portal diagnostics must both reference `door.slice`.

The suite then quicksaves the OPEN state, mutates the source door CLOSED, quickloads through the real Application/SaveCoordinator replacement path, and inspects the fresh world after its deferred navigation rebuild. The restored door must again be OPEN with a configured map-owned navigation link, open navigation passage, acoustic openness/portal door openness 1, clear production LOS through the doorway, a valid acoustic route, and zero pending semantic events. This proves restore rebuilds consumer state from semantic door truth rather than serializing engine/nav/acoustic machinery or replaying door consequences.

Focused manual acceptance remains required for 5.7 because the final requirement is player-readable coherence: guard door use must look physically plausible, a closed opaque door must simultaneously read as sight-blocking and sound-muffling, opening must visibly/audibly open the doorway connection without making the rotated leaf transparent, and fully open/partial leaves must still hide a player physically behind them. The current application exposes no player-facing quicksave/quickload control, so restore is intentionally not assigned as a manual action; the wired integration suite owns the open-door save/mutate/quickload proof through Application/SaveCoordinator. This manual check must not be used to retune the already accepted Phase 5 stealth values.
...
- the old slice-local footstep proof is now the shared three-tier system: carpet 0.09, stone 0.21, tile 0.90. The emitter queues semantic `gameplay.sound` facts and applies the shared 0.75 crouched multiplier, 1.0 standing multiplier, and 1.35 sprint/landing multiplier;
...
The application suite builds `res://missions/representative_stealth/mission.tres` through the real `MissionDefinition` / `WorldSession` path and therefore imports its authoritative TrenchBroom `.map` with the production Vark FGD. The regression requires the package to reach `PLAYING`, waits for the real navigation bake, and count-checks the representative authored roles so removed or duplicated starts, exits, openings, guard/patrol points, container, pickups, props, lights, switch, quiet/normal/loud surface volumes, or vertical-route checkpoints fail deterministically.

The current revision is deliberately an **irregular vertical city block**, not a flat compound. The source owns at least 100 separate world brushes across street floor, asymmetric building masses, door/window frames, rubble, market stalls, stoops, awnings, balconies, rooftop steps/bridges, chimneys, north-street barriers and a descending roof chain. Six semantic altitude checkpoints protect the intended progression bands: a mandatory south-street mantle, the elevated east key balcony, the raised west window, a mid/high rooftop line, the gold roof, and the north descent. CI also proves the gold loot and window are genuinely high, while final human acceptance remains responsible for proving those surfaces are actually comfortable/reachable with the accepted player controller.

The same proof verifies that the authored `window` presentation remains the shared ordinary-opening gameplay/save/navigation/acoustic spine, that the east opening is a real locked/key route, and that mission-start loot availability is latched as 2 pieces / 125 value before collection. It checks the live OmniLight emitter colors because FuncGodot mapper `Color` strings are byte-RGB values: the city lights must retain the luminous warm `GameplayLight` default and must not regress to near-black emitters. Collecting one real loot item must change collected progress without changing that available total. Collecting the real ledger must flow through `pickup.collected` into declarative objective completion and `alarm.raise_requested`, then through the existing alarm channel and guard communication/awareness owners into search. The resulting objective/exit truth and four-field mission-run loot state must be save-envelope truth without end-of-mission scene scraping.

Manual acceptance remains required. The representative mission exposes the existing application save/replacement owners directly as development hotkeys: **F5 = quicksave**, **F9 = quickload**, **F10 = restart current mission**. From **Development Launch → Representative Stealth**, complete the mission twice:

1. **West/window rooftop route:** leave the south gate and confirm the street itself immediately demands mantling instead of remaining flat. Follow the irregular west-side chain across rubble, carts/stalls, stoops, balconies and ledges to the raised ordinary window. Continue upward across the staggered roof pieces and small rooftop obstacles to the high gold roof, then use the separate descending roof/fire-escape chain to reach the north archive quarter. Height gains should arrive as many manageable steps rather than one impossible jump; no single missed mantle should strand the route.
2. **East/key street route:** after the shared south-street mantle, work up the scattered east market/fire-escape pieces to the elevated chest/key balcony. Take the brass service key and silver loot, descend through a different set of ledges, unlock the east ordinary door at street level, then continue north through repeated low street barriers/rubble to the ledger. This route must also require repeated climbing while remaining materially different from the west rooftop bypass.

Across both runs verify that the footprint reads as a **city**: crooked streets/alleys between buildings, changing building heights, scattered street furniture and cover, irregular silhouettes, no obvious mirrored grid, and frequent transitions between ground, low obstacles, balconies and roofs. Check that useful cover/LOS breaks remain plentiful; the six authored lights visibly illuminate nearby streets/building faces; the security switch darkens the center/north lamps; the east wall lamp can be directly extinguished; quiet/normal/loud surface semantics still differ; guard patrol waits and north-quarter investigation/search/navigation still work despite dense geometry; throwable-prop noise and typed speech remain useful; objective/exit/alarm behavior remains correct; a fresh F5 → F9 round-trip restores a meaningful mid-mission state; and F10 returns the mission to the authored **revision-3** start state. Report the exact location of any climb that is too high/too far, any surface that looks climbable but is not, any rooftop dead end, any route that can be bypassed on flat ground, or any street pocket that traps the player.

### Phase 8.4 alternate traversal-stress city

The accepted first vertical-city layout remains available unchanged as **Development Launch → Representative Stealth**. The second package, **Development Launch → Representative Stealth — Ledge City**, remains separate but its revision-2 layout is rebuilt as a coherent inhabited street block rather than a collection of climbing towers.

The spatial model now starts from believable city circulation: one continuous north/south boulevard wide enough for an ordinary patrol, a cross street, rear/service alleys, sidewalks, courtyards and five street-facing buildings. Mercer Row, the Watchmaker, Survey Office and the north tenement are hollow accessible buildings with front doors and divided ground-floor rooms. The Archive is a tall accessible objective building with a locked street entrance, internal rooms/mezzanines, a four-flight switchback stairwell and a high openable window into its upper objective room. The Watchmaker has a rooftop workroom containing the key/loot. Street clutter stays mostly on sidewalks/courtyards so the boulevard reads as intentional civic space rather than random obstacle scatter.

Traversal still uses the full accepted controller, but the surfaces are architectural: awnings, shop canopies, balconies, fire escapes, cornices, roof edges and window sills. The first required awning catch is about 2.5 m high. Repeated upward transfers rise about 1.0–1.06 m while staying within the tested hang-jump/catch envelope. Two materially separate same-height transfers cross the boulevard cross-street gap and the Archive service alley. The east route climbs the Watchmaker to a key room around 7 m high, then descends a separate rear fire escape. The west route climbs Mercer Row and Survey Office before scaling the Archive exterior to the high window. The locked-door route instead enters the Archive from the street and climbs its internal stairwell. Both converge in the Archive upper room roughly 14.25 m above street level.

The guard-route regression is stronger than a polygon-count check. The shared representative-world builder now queries the synchronized navigation graph between the authored patrol endpoints and refuses to become `navigation_ready` when they do not share a path. The Ledge City regression additionally queries that real path itself, shortens waits/increases speed in the isolated test instance, begins normal PLAYING simulation and requires the live guard to finish a patrol leg without a `last_error`. This specifically covers the previously observed runtime failure: `Guard 'guard.dev_stealth' has no route to patrol target 'patrol.rep.b'.`

Because revision 2 fundamentally changes spatial/save assumptions, **old Ledge City revision-1 quicksaves are intentionally incompatible**. Create a fresh F5 save after launching the new layout.

Manual acceptance for the second variant:

1. Launch **Representative Stealth — Ledge City** and first read the street. The center should look like a deliberate boulevard with building fronts/doors and a recognizable cross street, while rear edges form narrower service alleys. You should be able to enter the ordinary front doors of Mercer Row, the Watchmaker, Survey Office and north tenement and find actual divided interior rooms rather than solid building blocks.
2. **West/high-window approach:** catch the first high Mercer awning, climb its balconies/cornice, jump the real cross-street gap to Survey Office, continue up its facade/roof, jump the service alley to the Archive fire escape, then use repeated higher ledges to reach and open the Archive upper window. Reach the high gold and enter the upper objective room.
3. **East/key/interior approach:** enter/explore the Watchmaker, climb its street facade to the rooftop workroom, take the brass archive key and silver loot, descend by the rear fire escape into the service alley, return to the boulevard, unlock the Archive front door, then use the Archive interior switchback stairwell/mezzanines to reach the same upper objective room. This should feel materially different from the exterior-window route.
4. Watch the guard for at least one complete A→B patrol leg on the boulevard. There must be no `no route to patrol target` error. The 4 s / 5 s authored endpoint waits should still be visible in an ordinary playtest.
5. Recheck lights, cover/LOS, security switch, extinguishable interior lamps, surfaces, prop distraction, speech, loot/objective/alarm, a fresh F5→F9 round trip and F10 restart. Report any fake/non-enterable-looking door, room that traps the player, facade jump whose intended ledge is unclear, street segment that feels like arbitrary platforms rather than architecture, or patrol/search navigation failure.
...
## Interaction (Phase 6.1; preserves the Phase 3.1 contract)
...
- [ ] Center the Door Contract Probe while close: it visibly highlights even though the translucent **SENSOR AREA (non-blocking)** volume lies between the camera and door.
- [ ] Move the view off the Door Contract Probe: selection clears rather than using a nearby/off-axis object. Center it again and selection returns.
- [ ] Back outside interaction range and return: highlight clears/restores immediately and coherently.
- [ ] The Prop Contract Probe does not highlight through the solid divider; move around the divider until the center view has a clear physical line and it highlights.
- [ ] Press **F** on the highlighted Door Contract Probe: its `uses` count increments once; holding F does not repeatedly increment it; release and press again increments once more.
- [ ] Press **F** once on the Prop Contract Probe: it changes to `INACTIVE` and immediately loses interaction highlight, demonstrating current-state eligibility.
...
- [ ] Stand in the center door's sweep and close/open it as in the existing fixture: motion stops before entering the player and a fresh F reverses/resumes coherently after you clear it.
- [ ] Approach the left **LOCKED · requires key.lab** opening. It highlights through the normal selector, but repeated F without the key never starts physical opening.
- [ ] Approach the right **BARRED / externally restricted** opening. It highlights, but repeated F never starts physical opening.
- [ ] Restricted doors do not become invisible/non-interactable merely because access is denied.
- [ ] Movement, sprint, crouch, jump, and mouse look remain normal while using all three openings.
- [ ] Real player key acquisition/use is intentionally deferred to Phase 6.3; 6.2 automated coverage proves the door-side semantic key query/unlock contract.
...
- **Single truth:** switch and direct-extinguish operations call the same `VarkGameplayLight.set_enabled_state()`. ON/OFF synchronizes semantic exposure, actual child-emitter energy, and only the reusable fixture asset's designated lit-surface appearance. The body keeps its authored/imported PBR material untouched. The fixture asset owns an explicit `EmitterAnchor`, solid layer-1 player collision, a private physics-layer opaque-body trimesh used only by gameplay-light exposure rays, and a private-render-layer unshadowed self-fill at that same source. The coarse player collision is excluded from the fixture's own exposure ray so glass does not self-block; the opaque trimesh restores metal/frame occlusion. The self-fill never lights ordinary world geometry or contributes gameplay exposure.
