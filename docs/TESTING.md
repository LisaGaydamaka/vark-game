...
Focused manual acceptance remains required for 5.7 because the final requirement is player-readable coherence: guard door use must look physically plausible, a closed opaque door must simultaneously read as sight-blocking and sound-muffling, opening must visibly/audibly open that connection, and an open-door quickload must remain coherent in ordinary play. This manual check must not be used to retune the already accepted Phase 5 stealth values.
