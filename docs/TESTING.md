...
- the primitive guard tracks only the minimal door traversal states APPROACHING / WAITING_OPEN / CROSSING / CLEAR. While APPROACHING it uses a non-mutating probe of its own next physics-step movement and enters WAITING_OPEN only when that motion would collide with the configured ordinary door; then it consumes `is_navigation_passage_open()` and expresses an idempotent `request_open()` intent rather than using the player's toggle interaction. The ordinary door owns the physical body-in-passage query used to enter CROSSING;
...
- if the player commands CLOSE while the guard is still APPROACHING, the close is allowed to continue with no immediate AI OPEN counter-request while the guard still has clear intended movement; once the leaf physically blocks the guard's next movement, the guard starts/reuses the logical OPEN request and continues rather than remaining stranded;
