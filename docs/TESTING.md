...
- **Single truth:** switch and direct-extinguish operations call the same `VarkGameplayLight.set_enabled_state()`. ON/OFF synchronizes semantic exposure, actual child-emitter energy, and only the reusable fixture asset's designated lit-surface appearance. The body keeps its authored/imported PBR material untouched. The fixture asset owns an explicit `EmitterAnchor`, solid layer-1 collision, and a private-render-layer unshadowed self-fill at that same source so the frame can receive real lamp-origin lighting despite self-shadowing; that helper never lights ordinary world geometry or contributes gameplay exposure.
