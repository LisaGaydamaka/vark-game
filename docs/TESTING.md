...
- the primitive guard tracks only the minimal door traversal states APPROACHING / WAITING_OPEN / CROSSING / CLEAR. While APPROACHING, the ordinary door exposes the real horizontal sweep radius of its current collision leaf; the guard combines that with its own navigation/body radius and enters WAITING_OPEN only when its next intended physics step would cross that safe swing-clearance boundary. It then consumes `is_navigation_passage_open()` and expresses an idempotent `request_open()` intent rather than using the player's toggle interaction. The ordinary door also owns the physical body-in-passage query used to enter CROSSING;
...
- if the player commands CLOSE while the guard is still APPROACHING, the close is allowed to continue with no immediate AI OPEN counter-request while the guard remains outside the physical swing-clearance boundary; once its next step would enter that boundary, the guard stops outside the opening sweep, starts/reuses the logical OPEN request, and continues rather than becoming the blocker of its own requested opening;
