...
Each reached point still produces an explicit stop/scan period while ordinary hearing/vision remain live. Active search is bounded by both radius and total gameplay-time duration. Entering recovery releases navigation ownership to patrol immediately but leaves a gameplay-time residual-alert value. While that value remains high, the same locally heard sound can cross a temporarily reduced investigation threshold; the bonus decays to zero before fully unaware behavior.

The awareness semantic snapshot now stores the resolved candidate order plus stable seed, stage, uncertainty radius, confidence, search age, visited positions, scan progress, and residual alertness. The authoritative Awareness suite proves hidden-player motion cannot change resolved search truth, confidence decays only with simulation time, exhausted stages expand uncertainty, new evidence recenters/reseeds, quickload restores all already-resolved search semantics without reroll, residual alert decays while patrol ownership is restored, and the temporary re-alert threshold disappears at full unaware. Candidate aesthetics, scan cadence, and whether the controlled variation feels appropriately Thief-like remain focused user playtest territory.
