...
- the primitive guard tracks explicit door traversal states APPROACHING / WAITING_OPEN / REPOSITIONING_FOR_CLOSE / WAITING_CLOSE / REOPENING / CROSSING / CLEAR. The safe swing-clearance boundary controls where it may stop, while a separate physical query samples the guard's real capsule along the near-future NavigationAgent path against the ordinary door's **current collider transform**. WAITING_OPEN begins only when both the route is physically blocked and the safe stop boundary is reached; it retries idempotent `request_open()` only while that physical route remains blocked, and releases as soon as the current leaf angle becomes passable without requiring terminal OPEN. If a **fully open** leaf itself blocks a side approach before passage entry, the guard derives a same-side operating point from the real closed doorway frame and leaf sweep radius, repositions outside that sweep, requests idempotent CLOSE, then reopens and resumes the original patrol/awareness goal. The ordinary door owns both symmetric desired-state requests plus the physical body-in-passage and operating-point geometry seams; this is a bounded door maneuver, not general dynamic avoidance;
