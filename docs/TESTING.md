...
All awareness durations are remaining gameplay-simulation seconds advanced from `WorldSession.gameplay_time_seconds`. Weak locally propagated hearing can create suspicion; stronger heard evidence records an investigation target at the acoustic event origin. Footsteps retain the fixed 0.22 source-intensity investigation floor. Investigation-worthy hearing from unaware/suspicious/recovering/searching now enters `investigating` with navigation motion paused, turns the guard toward the evidence, and resolves a deterministic varied 0.80–2.00 s stare before movement begins; the investigation-stage timer itself does not count down during that stare. Vision resolves range/facing/LOS first, then accumulates a saved 0–1 visual-suspicion value instead of performing one-sample confirmation. Growth is continuous and exposure-dependent (0.10–1.60 per gameplay second across the production exposure range), decay is 0.35/s without useful sight, 0.35 starts investigation/stare, and 1.0 confirms alert. The legacy 0.28 confirmation field is now only the effective-exposure floor for the close darkness exception, so even point-blank darkness requires time to identify the player. Confirmed vision still owns local last-seen pursuit and the existing loss/pursuit/search model.
...
`tests/awareness/run_awareness_tests.gd` is an authoritative top-level suite using the real Integrated Slice. It proves weak hearing suspicion/decay, stop-turn-stare before strong heard investigation, resolved stare save truth, the exact production locomotion relationship (walking 1.20 < player sneak 2.00; investigating/search 0.72; pursuit run 2.64), continuous visual suspicion with bright exposure accumulating faster than the point-blank darkness path and neither confirming in one sample, visual stare before alert, search evidence interruption through the same stare gate, deterministic search/save truth, pursuit/loss/recovery, and vertical reacquisition. The suite shortens only timing values needed for CI. Compatibility fixtures follow the same semantics: Phase 4 tests accumulate vision until confirmed alert before snapshot/restore, the Phase 3 crouch-cover proof checks the final visibility/occlusion sample without requiring historical suspicion to vanish instantly, and the advanced vertical-attention proof isolates steep-angle geometry from production identification pacing.

Phase 5.4 had manual acceptance for the earlier instant-vision timing, but the continuous-suspicion, source-facing pre-investigation stare, and three-speed locomotion change intentionally reopens a focused manual gate. Prior structural acceptance for door/navigation, hearing, darkness, pursuit/loss, and recovery still stands; only the changed perception timing, stare cadence, and movement-speed feel need fresh user validation.
