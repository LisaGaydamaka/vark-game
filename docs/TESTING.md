...
- if the player commands CLOSE while the real guard is already in the doorway, guard door-use logic does not counter-request OPEN; the ordinary sweep advances to the last collision-free sample, physically contacts the guard, and remains stably latched partially closed without twitch/reopen;
