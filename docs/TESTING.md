...
`gameplay/noise/surface_profile.gd` defines the reusable authored surface-noise Resource. A valid profile has a non-empty semantic `surface_id` and exactly one loudness tier: `quiet`, `normal`, or `loud`. Footstep strength is derived canonically as 0.09 / 0.45 / 0.90 from that tier rather than authored as an arbitrary fourth value; a compatibility read property remains for existing callers. Footstep sound identity is still derived as `footstep.<surface_id>` so authoring cannot accidentally disagree about surface ID and semantic sound kind.
...
`tests/noise/run_noise_tests.gd` is an authoritative top-level suite. It validates the closed three-tier profile contract and fail-closed invalid IDs/tiers, reusable surface/profile composition, then launches the real Integrated Slice through Application/WorldSession. It verifies carpet = quiet/0.09, stone = normal/0.45, tile = loud/0.90, all three use derived `footstep.<surface>` kinds, even carpet run peaks below the real slice guard's tuned 0.16 hearing floor before attenuation, and the hearing-only behavior matrix is enforced with awareness vision frozen: close stone sneak is heard but remains suspicious-only even on repetition because its 0.2025 source strength is below the 0.22 investigation floor; stone walk/run investigate; tile sneak/walk/run investigate. The suite also preserves loudness-meter truth, gait multipliers, and the grounded burst-movement cadence regression. `tests/integration/run_phase3_slice_tests.gd` also requires all three profile-driven surfaces in the production slice wiring.
...
All awareness durations are remaining gameplay-simulation seconds advanced from `WorldSession.gameplay_time_seconds`. The production base visual range is 7.0 m (raised modestly from 6.0 m); this changes only the maximum range gate and does not widen horizontal FOV, vertical attention, bypass LOS/occlusion, alter gameplay exposure, or change suspicion rates. Weak locally propagated hearing can create suspicion. A newly heard suspicious cue from unaware/suspicious/recovering immediately uses an awareness-owned observation pause that stops patrol/door traversal motion, turns to the evidence, and holds without inventing a navigation route. That pause has bounded observation patience: production permits two stare freezes in a rapid cue chain, then further strong cues commit immediately to investigation and further weak cues cannot keep patrol frozen; a 6.0 s quiet gameplay-time window replenishes the allowance. The chain count and reset remainder are saved semantic truth. Investigation-worthy hearing from unaware/suspicious/investigating/recovering/searching otherwise enters or refreshes `investigating`, uses the deterministic varied 1.50–3.50 s source-facing hold, and only then permits investigating movement; the investigation-stage timer itself does not count down during that hold. Investigating/searching guards derive heightened evidence salience from state rather than from a second sensor: propagated hearing threshold and footstep source floor use 0.80× calm values, the minimum useful visual-exposure threshold uses 0.75× baseline, and visual-suspicion growth uses 1.50× rate. Acoustic propagation, physical hearing, range/facing/LOS/occlusion, and hidden player knowledge are unchanged. Vision still accumulates a saved 0–1 visual-suspicion value; baseline growth remains 0.10–1.60/s by exposure, decay remains 0.35/s whenever useful sight is lost (including during the hold), 0.35 starts investigation/hold, and 1.0 confirms alert. Confirmed ALERTED pursuit, including contact-grace and last-known checking, remains exempt from distraction holds and keeps its chase-specific evidence rules.
...
`tests/awareness/run_awareness_tests.gd` is an authoritative top-level suite using the real Integrated Slice. It proves the production 7.0 m base vision range, weak heard suspicion immediately stops/orients without creating a route, suspicious hold state is snapshot truth, two rapid investigation-worthy cues may use the hold but a third cannot restart it and instead commits immediately to navigation, the allowance replenishes after quiet gameplay time, the exact production 1.50–3.50 s hold and locomotion relationship (walking 1.20 < player sneak 2.00; investigating/search 0.72; pursuit run 2.64), visual suspicion can decay while the hold remains active, continuous bright-vs-dark suspicion pacing, search-state attention can promote a very close 0.2025 stone-sneak-strength cue below the calm 0.22 source floor, confirmed visible/lost pursuit remains exempt from distraction holds, deterministic search/save truth, pursuit/loss/recovery, and vertical reacquisition. The suite shortens only timing values needed for CI. Gameplay Noise continues proving repeated stone sneak from a calm/suspicious baseline remains capped below investigation; the Awareness suite separately proves prior stronger evidence can heighten interpretation without changing the surface emission or acoustic listener. Compatibility fixtures follow the same semantics.

Phase 5.4 remains under focused manual acceptance because the current TARGET tuning now adds longer source-facing holds, bounded repeat observation patience, plus state-dependent investigation/search attention. Prior structural acceptance for door/navigation, acoustic propagation, local knowledge, pursuit/loss ownership, and save semantics still stands; the user needs to validate immediate weak-cue stop/orient behavior, 1.50–3.50 s hold feel, heightened engaged evidence salience, suspicion decay during a hold, and the explicit exemption for confirmed/lost-pursuit chase behavior.
