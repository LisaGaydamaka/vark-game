...
- `vark_guard` and `vark_patrol_point` are exported through the ordinary Vark FGD/FuncGodot authoring path; the guard's current 2.0 m door-use value is a relevance cap for its configured ordinary door, not a standoff trigger;
...
- the primitive guard tracks explicit door traversal states APPROACHING / WAITING_OPEN / REPOSITIONING_FOR_CLOSE / WAITING_CLOSE / REOPENING / CROSSING / CLEAR, but **doorway traversal intent is authoritative before any of them may operate the door**. The ordinary door exposes its fixed closed-doorway center/normal/tangent/authored width and classifies a remaining NavigationAgent centerline as relevant only when it changes sides through the opening span; the classifier does not subtract agent radius a second time because the baked path is already radius-safe against static jamb geometry; walking beside the door, searching near it, or having the predicted capsule touch the swung leaf on the same side is not door use. Only after that crossing intent exists does the physical query sample the guard's real capsule along the near-future NavigationAgent path against the door's **current collider transform**. WAITING_OPEN begins only when that committed doorway route is physically blocked and the safe stop boundary is reached; it retries idempotent `request_open()` only while that physical route remains blocked, and releases as soon as the current leaf angle becomes passable without requiring terminal OPEN. If a **fully open** leaf blocks a committed side-to-side approach before passage entry, the guard derives a same-side operating point from the real closed doorway frame and leaf sweep radius, repositions outside that sweep, requests idempotent CLOSE, then reopens toward a geometry-derived opposite-side crossing point through the doorway center. The maneuver owns that crossing target until the body clears the passage, then restores the original patrol/awareness goal. The ordinary door owns the fixed doorway geometry, symmetric desired-state requests, and physical body-in-passage/operating-point seams; this is a bounded door traversal maneuver, not general dynamic avoidance;
...
- if the player commands CLOSE while the guard is still APPROACHING, the close is allowed to continue with no immediate AI OPEN counter-request while the current leaf remains physically passable. A deliberately 85%-open CLOSING leaf must stay passable with zero guard door-use requests; if the leaf actually blocks the near-future route, the guard stops at the safe boundary, starts/reuses the logical OPEN request, and resumes movement as soon as the physical route clears even if the semantic phase is still OPENING;
- if the player commands CLOSE while the real guard is already CROSSING the doorway, guard door-use logic does not counter-request before contact; the ordinary sweep advances to the last collision-free sample and records the real guard as the physical blocker. That confirmed guard-caused latch then authorizes the same logical OPEN intent, the door reverses without twitching, and the guard keeps moving as soon as the physical gap fits even while the door is still OPENING; once the guard clears the frame, a close behind it does not cause an unnecessary reopen;
...
The Application door regression also protects the consumer seam itself: `request_open()` does not toggle an opening/open door toward closed, a blocked opening remains physically safe, generic obstacle blockage stays latched, and re-requesting OPEN after the blocker leaves clears the obstruction latch without duplicating the original use sound. The ordinary door now exposes the symmetric idempotent `request_close()` seam used only by the bounded NPC maneuver; repeated CLOSE while closing only releases a transient obstruction latch and never toggles the door back open. The ordinary sweep commits the last collision-free probe position before latching rather than discarding the whole physics-step movement. It also protects the established shared full-width ordinary-door collision leaf. Guard/Nav Lab's zero-gap mapper-fit fixture handles its special frame tolerance locally by duplicating only that instance's collision shape and leaving 4 cm of sweep clearance at each vertical edge; the 1.30 m visible leaf still meets the authored jambs exactly, while the Navigation regression confirms a real player-close window with no immediate AI reopen, physical route blockage checked from the current leaf transform rather than terminal door phase, WAITING_OPEN releasing before terminal OPEN when the guard can fit, an 85%-open CLOSING leaf traversed with zero AI door use, a CROSSING guard allowing the close to reach real contact before issuing a contact-triggered OPEN request, then clearing the doorway before terminal OPEN, and the same traversal remaining one logical door use. Dedicated fully-open regressions now cover both sides of the classifier: one real baked route crosses the doorway and proves exactly one close/reposition/reopen maneuver reaches CROSSING without an infinite OPEN retry; another real NavigationAgent route remains on one doorway side while its capsule prediction touches the open leaf and proves zero door-use/maneuver requests. Fixed doorway geometry also gets a direct same-side-vs-side-to-side route classification assertion.
