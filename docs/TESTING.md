...
The accepted first vertical-city layout remains available unchanged as **Development Launch → Representative Stealth**. A second package, **Development Launch → Representative Stealth — Ledge City**, is intentionally more aggressive about the already-accepted traversal controller. It has its own MissionDefinition, map source, mission ID and revision so its saves cannot be confused with the baseline city.

The second map is authored against the current traversal envelope rather than ordinary standing mantles alone. With the accepted player dimensions/settings, a jump followed by ledge catch can reach an edge roughly 2.61 m above the takeoff body origin; a directional jump from a hang can gain roughly 1.25 m of edge height; normal/sprint hang jumps launch horizontally at 2.5 / 4.0 m/s. The map keeps safety margin below those maxima: its first mandatory catch is about 2.5 m high, repeated upward roof gaps rise about 1.0–1.06 m, and representative horizontal gaps are about 0.8–2.25 m. Two same-height skyline transfers are deliberately around 1.6–2.4 m so ledge-to-ledge directional jumping is materially useful.

Automated coverage builds the second package through the production WorldSession path, proves the original launch target still points to the preserved baseline mission, requires at least 105 independent world brushes and the representative gameplay roles, checks mapper-authored edge-pair markers against the conservative traversal envelope above, proves the high window/key/loot/objective contracts, waits for a valid guard navigation bake, reaches PLAYING, and verifies mission-start loot accounting.

Manual acceptance for the second variant is separate from the preserved baseline:

1. Launch **Representative Stealth — Ledge City**. The first west/east roof should not be reachable by simply walking into it and pressing mantle from the ground. Jump toward the edge, catch/hang, then mantle it.
2. **West skyline:** continue through the zig-zag roof islands. Several next roofs are more than the 0.75 m jump apex above the current roof, so you should need to jump, catch the higher ledge, hang, and mantle. Two transfers keep approximately the same height but open a larger physical gap; use a directional jump from the ledge/hang rather than treating them as touching platforms. Open/use the raised west window, reach the high gold, and continue to the archive roof.
3. **East/key route:** climb the separated south-east roof sequence to the high chest/key roof, descend by the alternate ledges, unlock the street gate, then climb the north skyline. It should again alternate jump-catch-mantles and gap transfers before converging on the archive apex.
4. Verify the final sealed ledger is genuinely high (about 14 m above street level), but every required transition remains readable and recoverable. Report the exact pair of surfaces for any jump that cannot be caught, any gap that can be skipped by walking/ordinary mantle, any place where the ledge detector grabs an unintended building instead, and any fall that leaves no reasonable recovery route.
5. Recheck lights, guard search/navigation, key/door, loot/objective/alarm, F5→F9 and F10. These systems intentionally remain the same representative contracts as the baseline city.
