...
The same proof verifies that the authored `window` presentation remains the shared ordinary-opening gameplay/save/navigation/acoustic spine, that the east opening is a real locked/key route, and that mission-start loot availability is latched as 2 pieces / 125 value before collection. It also checks the live OmniLight emitter colors, because FuncGodot mapper `Color` strings are byte-RGB values: the representative mission must retain the luminous warm `GameplayLight` default and must not regress to near-black emitters from hand-authored 0–1 color strings. Collecting one real loot item must change collected progress without changing that available total. Collecting the real ledger must flow through `pickup.collected` into declarative objective completion and `alarm.raise_requested`, then through the existing alarm channel and guard communication/awareness owners into search. The resulting objective/exit truth and four-field mission-run loot state must be save-envelope truth without end-of-mission scene scraping.

Manual acceptance remains required for subjective/use-path behavior. The representative mission exposes the existing application save/replacement owners directly as development hotkeys: **F5 = quicksave**, **F9 = quickload**, **F10 = restart current mission**. From **Development Launch → Representative Stealth**, complete the mission twice:

1. **West/window + upper route:** take the quiet west service passages. The route/climb crates and stepped platforms must support repeated mantles up to the raised ordinary-window crossing rather than one cosmetic hop. Open the window, continue onto the elevated shelves/catwalk/perches, and verify the optional gold loot is genuinely high above the ground route. The upper line should provide useful alternate sightlines/avoidance and a believable descent toward the north archive, not strand the player or collapse into an invisible-wall puzzle.
2. **East/key room chain:** take the loud east side through the smaller connected rooms, open/search the chest, take the brass service key, reach/unlock the east ordinary door, and continue through the central/north room network to the ledger. Confirm this remains a materially distinct ground route rather than a cosmetic detour.

Across both runs verify the same-size map now feels like interconnected passages/rooms instead of a few large open zones; pillars, low partitions, props, corners and alcoves provide practical places to break line of sight/hide; the six authored lights make the baseline route readable; the start/west path remains visible without flattening all shadow; the security switch meaningfully darkens the center/north street lamps; the east wall lamp can still be directly extinguished; center stone is normal relative to quiet carpet/loud tile; guard patrol waits occur and the tighter geometry does not break investigation/search/navigation; throwable-prop noise and typed speech remain useful; objective/exit/alarm behavior remains correct; F5 followed by F9 restores a meaningful mid-mission state without freezing; and F10 returns the mission to the authored revision-2 start state. Report any blocked climb, unreachable perch/loot, route that becomes redundant, lightless area that prevents navigation, or hiding geometry that traps the player.
