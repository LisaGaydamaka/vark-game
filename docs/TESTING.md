...
All awareness durations are remaining gameplay-simulation seconds advanced from `WorldSession.gameplay_time_seconds`. Production keeps continuous sight and the source-facing stare architecture while using Thief-like tuning: base visual range 10.0 m; two stare freezes per rapid cue chain with a 12.0 s reset; engaged hearing and footstep floors stay at 1.00× calm; visual interpretation uses a 0.90× exposure floor and 1.25× suspicion growth; baseline visual growth is 0.10–2.00/s, decay 0.08/s, 0.40 starts investigation/hold, and 1.0 confirms alert. Suspicion lasts 4 s, investigation 12 s, search 45 s, lost pursuit 30 s, and recovery 8 s. Confirmed ALERTED pursuit remains exempt from distraction holds.
