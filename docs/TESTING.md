...
- TrenchBroom FGD export for `vark_acoustic_space` and `vark_acoustic_portal`, including a unit-size acoustic-space editor model and a property-driven space-delimited XYZ scale string; the regression explicitly rejects the numeric-array form because TrenchBroom 2026.2 interprets scale arrays as fallback candidates, not vector components;
- real FuncGodot import of IDs, portal links, transmission tuning, and mapper-unit extents;
- explicit protection that the current authoritative map scale is 32 mapper units per Vark world meter and that, for example, 96/64/128 authored half-extents become 3/2/4 world meters.

The development Acoustic Lab supplies the focused player-facing proof with an open room, door-separated room, disconnected room, and two-portal corner route. Manual acceptance still decides whether the current attenuation/transmission relationships feel intuitive and whether the explicit mapper topology cost is acceptable. The first TrenchBroom 2026.2 mapper pass failed because acoustic-space extents were editable but visually invisible. The next pass exposed the deeper model-expression issue: the cage appeared, but a numeric `[X, Z, Y]` scale expression is a fallback list in TrenchBroom, so only X produced uniform cube scaling. The current authoring contract therefore requires a unit cage driven by one space-delimited vector-string expression, with independent X/Y/Z resizing and save/reopen survival before Phase 3.6 can be accepted. Do not stabilize the Phase 5 production acoustic API or tuning from the deterministic spike alone.
