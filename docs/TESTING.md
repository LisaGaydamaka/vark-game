...
- if the player commands CLOSE while the real guard is already CROSSING the doorway, guard door-use logic does not counter-request OPEN; the ordinary sweep advances to the last collision-free sample, physically contacts the guard, and remains stably latched partially closed without twitch/reopen; once the guard clears the frame, a close behind it does not cause an unnecessary reopen;
