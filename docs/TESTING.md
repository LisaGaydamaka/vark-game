...
`gameplay/noise/surface_profile.gd` defines the reusable authored surface-noise Resource. A valid profile has a non-empty semantic `surface_id` and exactly one loudness tier: `quiet`, `normal`, or `loud`. Footstep strength is derived canonically as 0.20 / 0.52 / 0.80 from that tier rather than authored as an arbitrary fourth value; a compatibility read property remains for existing callers. Footstep sound identity is still derived as `footstep.<surface_id>` so authoring cannot accidentally disagree about surface ID and semantic sound kind.
...
`tests/noise/run_noise_tests.gd` is an authoritative top-level suite. It validates the closed three-tier profile contract and fail-closed invalid IDs/tiers, reusable surface/profile composition, then launches the real Integrated Slice through Application/WorldSession. It verifies carpet = quiet/0.20, stone = normal/0.52, tile = loud/0.80, all three use derived `footstep.<surface>` kinds and reach the real guard acoustic listener, the development loudness meter reports the same tier/value truth, crouch/sprint remain gait multipliers rather than extra surface classes, and repeated grounded sub-step movement bursts separated by full stops preserve cadence distance and eventually emit instead of resetting to silence. `tests/integration/run_phase3_slice_tests.gd` also requires all three profile-driven surfaces in the production slice wiring.
