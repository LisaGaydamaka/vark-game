...
`tests/noise/run_noise_tests.gd` is an authoritative top-level suite. It validates the closed three-tier profile contract and fail-closed invalid IDs/tiers, reusable surface/profile composition, then launches the real Integrated Slice through Application/WorldSession. It verifies carpet = quiet/0.09, stone = normal/0.45, tile = loud/0.90, all three use derived `footstep.<surface>` kinds, normal carpet walking at about one meter is below the 0.08 listener floor while carpet sprinting rises above it, and stone/tile reach the real guard acoustic listener, the development loudness meter reports the same tier/value truth, crouch/sprint remain gait multipliers rather than extra surface classes, and repeated grounded sub-step movement bursts separated by full stops preserve cadence distance and eventually emit instead of resetting to silence. `tests/integration/run_phase3_slice_tests.gd` also requires all three profile-driven surfaces in the production slice wiring.
