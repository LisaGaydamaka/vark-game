...
The awareness semantic snapshot now stores the resolved candidate order plus stable seed, stage, uncertainty radius, confidence, search age, visited positions, current search action, action duration/remaining time, look directions/count, resolved movement-speed scale, and residual alertness. The authoritative Awareness suite proves hidden-player motion cannot change resolved truth, confidence/uncertainty progression is simulation-time based, production radius/separation defaults are widened, resolved stops honor configured horizontal spacing, search locomotion is slower than patrol while pursuit is faster, stationary search actions actually pause navigation motion, quickload restores the already-resolved action instead of rerolling timing/pose, elevated evidence remains vertically explicit while candidate endpoints stay truly reachable, steep elevated-player vision is rejected in unaware state but can be reacquired during widened search/alert vertical attention, and residual alert still decays/re-alerts correctly. Exact cadence, patience, look presentation, speed ratios, and final vertical-attention feel remain focused user playtest territory.
