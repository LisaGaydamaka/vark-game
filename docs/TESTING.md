...
`tests/awareness/run_awareness_tests.gd` is an authoritative top-level suite using the real Integrated Slice. It proves weak hearing suspicion/decay, stop-turn-stare before strong heard investigation, resolved stare save truth, the exact production locomotion relationship (walking 1.60 < player sneak 2.00; investigating/search 0.72; pursuit run 2.64), continuous visual suspicion with bright exposure accumulating faster than the point-blank darkness path and neither confirming in one sample, visual stare before alert, search evidence interruption through the same stare gate, deterministic search/save truth, pursuit/loss/recovery, and vertical reacquisition. The suite shortens only timing values needed for CI. Compatibility fixtures follow the same semantics: Phase 4 tests accumulate vision until confirmed alert before snapshot/restore, the Phase 3 crouch-cover proof checks the final visibility/occlusion sample without requiring historical suspicion to vanish instantly, and the advanced vertical-attention proof isolates steep-angle geometry from production identification pacing.
