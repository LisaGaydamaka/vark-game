...
- the primitive guard tracks only the minimal door traversal states APPROACHING / WAITING_OPEN / CROSSING / CLEAR. The safe swing-clearance boundary controls where it may stop, while a separate physical query samples the guard's real capsule along the near-future NavigationAgent path against the ordinary door's **current collider transform**. WAITING_OPEN begins only when both the route is physically blocked and the safe stop boundary is reached; it retries idempotent `request_open()` only while that physical route remains blocked, and releases as soon as the current leaf angle becomes passable without requiring terminal OPEN. The ordinary door also owns the physical body-in-passage query used to enter CROSSING;
...
- automated timing proves text remains hidden before the controlled semantic consequence pass. A single active cover-started utterance then moves through multiple farther positions with fill/outline alpha locked together and strictly decreasing nonzero values; five additional closely spaced near-threshold samples must each decrease by less than 0.04 alpha and approach effectively zero before the inaudible boundary, then the same utterance reaches hidden at the inaudible marker and becomes visible again after returning to audible range while semantic queued/heard counts remain unchanged; a short line proves lifetime expiry clears the text;
