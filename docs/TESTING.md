...
Focused manual acceptance remains required for 5.7 because the final requirement is player-readable coherence: guard door use must look physically plausible, a closed opaque door must simultaneously read as sight-blocking and sound-muffling, and opening must visibly/audibly open that connection. The current application exposes no player-facing quicksave/quickload control, so restore is intentionally not assigned as a manual action; the wired integration suite owns the open-door save/mutate/quickload proof through Application/SaveCoordinator. This manual check must not be used to retune the already accepted Phase 5 stealth values.
