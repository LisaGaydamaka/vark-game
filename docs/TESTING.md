...
- TrenchBroom FGD export for `vark_acoustic_space` and `vark_acoustic_portal`;
...
The development Acoustic Lab supplies the focused player-facing proof with an open room, door-separated room, disconnected room, and two-portal corner route. Manual acceptance still decides whether the current attenuation/transmission relationships feel intuitive and whether the explicit mapper topology cost is acceptable. Do not stabilize the Phase 5 production acoustic API or tuning from the deterministic spike alone.
