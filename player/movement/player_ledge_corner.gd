...
func find_same_wall_connection(
	...
	# Smooth continuation belongs to normal shimmy. A same-wall transition is
	# only for a real local tangent break, not a bridge across a gap.
...
func find_wall_turn_connection(
	...
	var discovery_tolerance: float = (
		endpoint_resolution
		+ detector.get_top_probe_inset()
		+ get_connection_tolerance()
	)
...
func find_bounded_ledge_connection(
	...
	if (
		source_horizontal_bound < 0.0
		or target_backward_horizontal_bound < 0.0
		or target_forward_horizontal_bound < 0.0
		or connection_tolerance < 0.0
	):
		return {}
	...
	if (
		source.length_squared() <= MOTION_EPSILON_SQUARED
		or target.length_squared() <= MOTION_EPSILON_SQUARED
	):
		return {}
	...
	var source_parameter: float = (
		(source_target_dot * target_between_dot - source_between_dot)
		/ denominator
	)
	var target_parameter: float = (
		(target_between_dot - source_target_dot * source_between_dot)
		/ denominator
	)
	...
	if absf(source_horizontal_progress) > source_horizontal_bound:
		return {}
	if (
		target_horizontal_progress < -target_backward_horizontal_bound
		or target_horizontal_progress > target_forward_horizontal_bound
	):
		return {}
...
func target_reaches_connection(
	...
	var sample_point: Vector3 = (
		connection_point
		+ target_direction * (near_horizontal_distance / horizontal_factor)
	)
...
func find_local_ledge_candidate(
	...
	return detector.find_local_candidate(
		player,
		support,
...
func find_wall_plane_corner_point(
	...
	var corner_x: float = (
		(source_distance * target_normal.z - source_normal.z * target_distance)
		/ determinant
	)
	var corner_z: float = (
		(source_normal.x * target_distance - source_distance * target_normal.x)
		/ determinant
	)
...
func try_start(player: CharacterBody3D, candidate: CornerCandidate) -> bool:
	...
	arc_length = (
		get_arc_radius() * absf(candidate.signed_turn_angle)
		if candidate.changes_wall
		else 0.0
	)
	target_lead_length = candidate.target_arc_position.distance_to(
...
func is_expected_corner_contact(
	...
	var collision_normal: Vector3 = _horizontal_normal(
	...
	return (
		active_corner.changes_wall
		and is_point_near_arc_region(collision_point)
		and is_normal_in_corner_sector(collision_normal)
	)
...
func get_route_position(distance_along_route: float) -> Vector3:
	...
	if arc_length > 0.000001 and arc_distance <= arc_length:
		...
		arc_position.y = (
			active_corner.corner_point.y
			- detector.get_hang_anchor_height()
		)
		return arc_position
	...
	var target_distance: float = (
		clamped_distance - source_lead_length - arc_length
	)
...
func get_oriented_wall_tangent(
	...
	var preferred: Vector3 = _horizontal_normal(preferred_direction)
	if (
		preferred.length_squared() > MOTION_EPSILON_SQUARED
