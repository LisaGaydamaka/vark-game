...
func _init(p_turn_speed_degrees: float, p_detector: PlayerLedgeDetector) -> void:
...
func find_candidate(
	...
	var source_normal := Vector3(segment_wall_normal.x, 0.0, segment_wall_normal.z)
	if source_normal.length_squared() <= MOTION_EPSILON_SQUARED:
		return null
	source_normal = source_normal.normalized()

	var source_travel := Vector3(shimmy_direction.x, 0.0, shimmy_direction.z)
	if source_travel.length_squared() <= MOTION_EPSILON_SQUARED:
		return null
	source_travel = source_travel.normalized()
	if absf(source_normal.dot(source_travel)) > sin(deg_to_rad(MAX_RIGHT_ANGLE_ERROR_DEGREES)):
		return null

	var source_path_direction: Vector3 = get_oriented_ledge_direction(
		source_candidate.ledge_direction,
		source_travel
	)
	if source_path_direction.length_squared() <= MOTION_EPSILON_SQUARED:
		return null
	...
	var endpoint_result: Dictionary = find_source_endpoint(
		...
		source_candidate,
		source_normal,
		source_path_direction
	)
	if endpoint_result.is_empty():
		return null

	var endpoint_value: Variant = endpoint_result.get("point")
	var endpoint_resolution_value: Variant = endpoint_result.get("resolution")
	if not (endpoint_value is Vector3):
		return null
	if typeof(endpoint_resolution_value) != TYPE_FLOAT and typeof(endpoint_resolution_value) != TYPE_INT:
		return null
	var endpoint_guess: Vector3 = endpoint_value
	var endpoint_resolution: float = float(endpoint_resolution_value)
	...
	var target_probe_point: Vector3 = endpoint_guess + expected_target_continuation * target_probe_inset
	var target_wall: PlayerLedgeDetector.WallHit = detector.find_wall_near_edge(
		player,
	...
	if target_wall.normal.dot(expected_target_normal) < minimum_target_alignment:
		return null

	var signed_turn_angle: float = get_signed_horizontal_angle(source_normal, target_wall.normal)
	...
	if (
		...
		or absolute_turn_degrees > 90.0 + MAX_RIGHT_ANGLE_ERROR_DEGREES
	):
		return null
	...
	var corner_point_result: Dictionary = find_wall_plane_corner_point(
	...
	if corner_point_result.is_empty():
		return null
	var corner_point_value: Variant = corner_point_result.get("point")
	if not (corner_point_value is Vector3):
		return null
	var corner_point: Vector3 = corner_point_value
	var corner_guess_delta := Vector3(
		corner_point.x - endpoint_guess.x,
		0.0,
		corner_point.z - endpoint_guess.z
	)
	if corner_guess_delta.length() > endpoint_tolerance:
		return null

	target_probe_point = corner_point + target_continuation * target_probe_inset
	...
	signed_turn_angle = get_signed_horizontal_angle(source_normal, target_wall.normal)
	...
	var target_edge_point := Vector3(
		target_wall.point.x,
		target_top.point.y,
		target_wall.point.z
	)
	if not detector.is_edge_exposed(player, target_edge_point, target_wall.normal):
		return null

	var target_from_corner := Vector3(
		target_edge_point.x - corner_point.x,
		0.0,
		target_edge_point.z - corner_point.z
	)
	var target_progress: float = target_from_corner.dot(target_continuation)
	if target_progress < target_probe_inset - endpoint_tolerance:
		return null

	var target_ledge_axis: Vector3 = detector.get_ledge_direction(
		target_wall.normal,
		target_top.normal
	)
	target_ledge_axis = get_oriented_ledge_direction(
		target_ledge_axis,
		target_continuation
	)
	if target_ledge_axis.length_squared() <= MOTION_EPSILON_SQUARED:
		return null

	var target_horizontal_axis := Vector3(
		target_ledge_axis.x,
		0.0,
		target_ledge_axis.z
	)
	var target_horizontal_factor: float = target_horizontal_axis.length()
	if target_horizontal_factor <= 0.000001:
		return null
	target_horizontal_axis /= target_horizontal_factor
	...
	var target_hang_position: Vector3 = detector.get_hang_position(
		target_edge_point,
		target_wall.normal
	)
	var target_candidate := PlayerLedgeDetector.LedgeCandidate.new()
	...
	var corner_hang_height: float = corner_point.y - detector.get_hang_anchor_height()
	var source_arc_position: Vector3 = corner_point + source_normal * arc_radius
	source_arc_position.y = corner_hang_height
	var target_arc_position: Vector3 = corner_point + target_wall.normal * arc_radius
	target_arc_position.y = corner_hang_height
	...
	candidate.target_continuation_direction = target_continuation
...
func find_source_endpoint(
	...
	for scan_index: int in range(1, CORNER_ENDPOINT_SCAN_STEPS + 1):
		var sample_distance: float = (
			search_distance * float(scan_index) / float(CORNER_ENDPOINT_SCAN_STEPS)
		)
	...
	for _iteration: int in range(CORNER_ENDPOINT_SEARCH_STEPS):
...
func source_wall_exists_at_distance(
	...
	var sample_point: Vector3 = get_ledge_point_at_horizontal_distance(
		source_candidate.edge_point,
		travel_direction,
		distance
	)
	var sample_wall: PlayerLedgeDetector.WallHit = detector.find_wall_near_edge(
		player,
		source_normal,
		sample_point
	)
	return (
		sample_wall != null
		and detector.is_wall_continuous(sample_wall, source_normal, sample_point)
	)
...
func get_ledge_point_at_horizontal_distance(
	...
	var horizontal_factor: float = horizontal.length()
...
func get_oriented_ledge_direction(
	...
	direction = direction.normalized()
	var horizontal := Vector3(direction.x, 0.0, direction.z)
	var preferred := Vector3(preferred_direction.x, 0.0, preferred_direction.z)
	if (
		horizontal.length_squared() <= MOTION_EPSILON_SQUARED
		or preferred.length_squared() <= MOTION_EPSILON_SQUARED
	):
		return Vector3.ZERO
...
func find_wall_plane_corner_point(
	...
	var corner_x: float = (
		(source_distance * target_normal.z - source_normal.z * target_distance) / determinant
	)
	var corner_z: float = (
		(source_normal.x * target_distance - source_distance * target_normal.x) / determinant
	)
...
func try_start(player: CharacterBody3D, candidate: CornerCandidate) -> bool:
	...
	arc_length = get_arc_radius() * absf(candidate.signed_turn_angle)
...
func simulate_corner_to_position(
	...
	for _iteration: int in range(PROBE_MAX_COLLISIONS):
		...
		if (
			...
			and next_motion.length_squared() >= previous_length_squared - MOTION_EPSILON_SQUARED
		):
			return {}
...
func is_expected_corner_contact(
	...
	if not is_normal_in_corner_sector(collision_normal):
		return false
...
func is_point_near_candidate_wall_plane(
	...
	var plane_distance: float = absf((point - candidate.edge_point).dot(wall_normal))
...
func is_normal_in_corner_sector(normal: Vector3) -> bool:
	if active_corner == null:
		return false
	var normal_angle: float = get_signed_horizontal_angle(active_corner.source_wall_normal, normal)
...
func get_oriented_wall_tangent(
	...
	var tangent: Vector3 = Vector3.UP.cross(wall_normal)
	tangent.y = 0.0
...
func get_signed_horizontal_angle(from_normal: Vector3, to_normal: Vector3) -> float:
	var normalized_from := Vector3(from_normal.x, 0.0, from_normal.z)
	var normalized_to := Vector3(to_normal.x, 0.0, to_normal.z)
	if (
		normalized_from.length_squared() <= MOTION_EPSILON_SQUARED
		or normalized_to.length_squared() <= MOTION_EPSILON_SQUARED
	):
		return 0.0
