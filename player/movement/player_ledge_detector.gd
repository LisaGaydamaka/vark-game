...
func update_suppression(player: CharacterBody3D) -> void:
	...
	var horizontal_edge_offset: Vector3 = Vector3(edge_offset.x, 0.0, edge_offset.z)
...
func find_candidate(
	...
	var horizontal_intent: Vector3 = Vector3(intent_direction.x, 0.0, intent_direction.z)
	...
	var edge_point: Vector3 = Vector3(wall_hit.point.x, top_hit.point.y, wall_hit.point.z)
	var near_edge_wall: WallHit = find_wall_near_edge(player, wall_hit.normal, edge_point)
	if near_edge_wall == null or not is_wall_continuous(near_edge_wall, wall_hit.normal, edge_point):
		return null

	edge_point = Vector3(near_edge_wall.point.x, top_hit.point.y, near_edge_wall.point.z)
	var edge_offset: Vector3 = edge_point - player.global_position
	var horizontal_edge_offset: Vector3 = Vector3(edge_offset.x, 0.0, edge_offset.z)
	...
	var relative_height: float = top_hit.point.y - player.global_position.y
	if relative_height < get_min_edge_height() - PROBE_SAFE_MARGIN:
		return null
	if relative_height > get_max_catch_height() + PROBE_SAFE_MARGIN:
		return null

	var ledge_direction: Vector3 = Vector3.UP.cross(near_edge_wall.normal)
	...
	var hang_position: Vector3 = get_hang_position(edge_point, near_edge_wall.normal)
	...
	if hang_span_valid:
		candidate.hangable = is_hang_pose_valid(player, candidate, hang_position)
...
func find_hang_candidate_at_position(
	...
	var edge_point: Vector3 = Vector3(wall_hit.point.x, top_hit.point.y, wall_hit.point.z)
	var hang_position: Vector3 = get_hang_position(edge_point, wall_hit.normal)
	if absf(hang_position.y - proposed_hang_position.y) > get_shimmy_level_tolerance():
		return null
	...
	if horizontal_correction.length_squared() > correction_limit * correction_limit:
		return null

	var ledge_direction: Vector3 = Vector3.UP.cross(wall_hit.normal)
	if ledge_direction.length_squared() <= MOTION_EPSILON_SQUARED:
		return null
	ledge_direction = ledge_direction.normalized()
	var reference_ledge_direction: Vector3 = Vector3.UP.cross(fixed_wall_normal).normalized()
...
func find_wall(player: CharacterBody3D, approach_direction: Vector3) -> WallHit:
	...
	for collision_index: int in range(collision_count):
		...
		var horizontal_normal: Vector3 = Vector3(collision_normal.x, 0.0, collision_normal.z)
...
func find_wall_near_edge(
	...
	var ray_from: Vector3 = expected_edge_point + expected_wall_normal * (
		get_hang_wall_distance() + get_top_probe_inset()
	)
	...
	var query: PhysicsRayQueryParameters3D = _prepare_ray_query(player, ray_from, ray_to)
...
func has_catch_intent(
	...
	var horizontal_view: Vector3 = Vector3(view_forward.x, 0.0, view_forward.z)
...
func _is_ledge_span_sample_valid(
	...
	var sample_edge_point: Vector3 = edge_point + ledge_direction * half_width * direction_sign
	...
	var top_hit: TopHit = find_top_for_hang(player, support, wall_hit, edge_height)
	if top_hit == null:
		return false
	return absf(top_hit.point.y - edge_height) <= get_shimmy_level_tolerance()
...
func is_expected_hang_wall_contact(
	...
	return collision_normal.dot(expected_wall_normal) >= EXPECTED_HANG_WALL_MIN_ALIGNMENT
