...
if start < 0:
    raise SystemExit("moving-prop mantle regression start marker not found")
end = text.find(end_marker, start)
if end < 0:
    raise SystemExit("moving-prop mantle regression end marker not found")
...
\t\t"Grounded mantle pressed while pushing a moving prop enters PlayerMantle instead of ballistic jump"
\t)
...
\tassert_true.call(
\t\tcompleted_mantle,
\t\t"Ground mantle completes through the ordinary PlayerMantle lift/forward path on the prop"
\t)
\tassert_true.call(
...
print("CORRECTED_MOVING_PROP_MANTLE_REGRESSION_SEQUENCE")
